...
test("core runtime builtins seed generic guidance target metadata without demo or mcp authoring extras", () => {
  ...
  assert.equal(Boolean(model.valueTypesById["widget.guidanceTarget"]), true);
  ...
  assert.equal(processSpecIds.has("guidance_widget_target_spec"), false);
...
test("profile seed rows alone do not install plugin-owned builtin seeds", () => {
  ...
  assert.equal(specs.has("guidance_widget_target_spec"), false);
  assert.equal(specs.has("tutorial_widget_target_spec"), false);
});

test("plugin-owned runtime builtin seeds install when contributed by active plugins without re-owning guidance targeting", () => {
  const world = createWorld();
  ensureRuntimeBuiltins(world, {
    capabilityIds: [],
    seedContributions: [
