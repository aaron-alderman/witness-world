...
test("governance inventory preserves proposal, operator, mixed, and session classifications", async () => {
  ...
  assert.equal(byHandler.get("frontend.migrateLegacy")?.governanceMode, "proposal-fallback");
...
test("proposal target governance catalog covers every supported executor target and drives bootstrap-selectable targets", async () => {
  ...
  assert.equal(proposalTargetProcessIds({ bootstrapSelectableOnly: true }).includes("frontend.migrateLegacy"), true);
  ...
  assert.equal(proposalTargetGovernanceEntry("frontend.migrateLegacy")?.governanceMode, "proposal-fallback");
