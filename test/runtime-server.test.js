...
test("runtime server serves Engentus generated stylesheet routes from a snapshot-scoped cache", async () => {
  const world = createWitnessWorld();
  const runner = {
    id: "runner-1",
    backendHost: "backendHost",
  ...
  const server = await startRuntimeServer(world, {
    ...
    loadEngentusGeneratedCssBundle: async () => {
      loadCount += 1;
      return {
        files: {
          "engentus-shell.css": `/* shell rev ${activeSnapshot.appRevision} */`,
          "engentus-chart-pages.css": `/* chart rev ${activeSnapshot.appRevision} */`
        }
      };
    },
    httpModule: {
  ...
  const shellReq = { method: "GET", url: "/engentus/__generated/engentus-shell.css", headers: {}, on() {} };
  const chartReq = { method: "GET", url: "/engentus/__generated/engentus-chart-pages.css", headers: {}, on() {} };

  const firstShell = createResponse();
  await requestHandler(shellReq, firstShell);
  assert.equal(firstShell.statusCode, 200);
  assert.equal(firstShell.body, "/* shell rev 1 */");
  assert.equal(firstShell.headers?.["content-type"], "text/css; charset=utf-8");

  const secondChart = createResponse();
  await requestHandler(chartReq, secondChart);
  assert.equal(secondChart.statusCode, 200);
  assert.equal(secondChart.body, "/* chart rev 1 */");
  assert.equal(loadCount, 1);

  activeSnapshot = { appRevision: 2, world };

  const thirdShell = createResponse();
  await requestHandler(shellReq, thirdShell);
  assert.equal(thirdShell.statusCode, 200);
  assert.equal(thirdShell.body, "/* shell rev 2 */");
