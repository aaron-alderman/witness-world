...
const browserRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "examples", "operator", "browser");
const defaultWorkspaceRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const defaultExampleRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "examples", "operator");
...
export async function startOperatorBrowserExampleServer({
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (requestUrl.pathname === "/api/snapshot" && method === "GET") {
        if (!ownedCore) {
      ...
      if (requestUrl.pathname === "/api/command" && method === "POST") {
        if (!ownedCore) {
          writeJson(res, 503, { error: "operator bridge unavailable in fixture-only mode" });
          return;
        }
      ...
      if (requestUrl.pathname === "/api/intent" && method === "POST") {
        if (!ownedCore) {
          writeJson(res, 503, { error: "operator bridge unavailable in fixture-only mode" });
          return;
        }
      ...
      if (requestUrl.pathname === "/api/display-settings" && method === "POST") {
        if (!ownedCore) {
          writeJson(res, 503, { error: "operator bridge unavailable in fixture-only mode" });
          return;
        }
