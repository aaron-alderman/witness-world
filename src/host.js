...
function createSearchIndexRuntime({ world, runtimeConfig, runtimeRoot, serverRunnerId, storage }) {
  ...
  const resolveAssetSource = async (descriptor, config) => {
    ...
    if (looksTextSearchMime(asset.mimeType)) {
      try {
  ...
  const handlers = {
    ...
    "search.index.query": async ({ req, res, requestActor, appContext }) => {
      ...
      const hits = result.hits.map(hit => ({
        ...hit,
        ...(hit.assetId ? { contentUrl: `/api/assets/${encodeURIComponent(hit.assetId)}/content` } : {})
      }));
      emitSearchIndexEvent({
        actor: requestActor,
    ...
    "edenVersions.read": async ({ res, route }) => {
    ...
    "edenVersions.activate": async ({ req, res, requestActor, route }) => {
      ...
      if (!result.ok) {
        sendJson(res, result.status, { error: result.error, witness: result.witness, versionState: result.versionState ?? null });
        return;
      }
      sendJson(res, 200, { ok: true, status: result.activationStatus, witness: result.witness, versionState: result.versionState });
    },
    ...
    "edenVersions.rollback": async ({ res, requestActor, route }) => {
      ...
      sendJson(res, 200, { ok: true, status: result.rollbackStatus, witness: result.witness, versionState: result.versionState });
    },
    ...
    "edenVersions.publish": async ({ req, res, requestActor, route }) => {
      ...
      sendJson(res, 200, { ok: true, witness: result.witness, versionState: result.versionState });
    },
    ...
    "asset.content.read": async ({ res, params, requestActor, requestUrl, appContext }) => {
      ...
      if (!isPublic) {
        if (!requestActor) {
          world.observe({ process: "asset.content.read.failed", actor: backendHost, claims: [], body: { id: params.id || "", reason: "no actor" } });
          sendJson(res, 401, { error: "sign in first" });
          return;
        }
        const gate = canMutateTarget(world, requestActor, asset.id);
        if (!gate.ok) {
          world.observe({ process: "asset.content.read.failed", actor: requestActor, claims: [], body: { id: asset.id, reason: gate.reason } });
          sendJson(res, gate.status || 403, { error: gate.reason || "forbidden" });
          return;
        }
      }
