...
function renderAttributes(surface, tagName, options = {}) {
  ...
  const attrs = [];
  const authoredSurfaceId = staticTextValue({ value: surface?.id }, "value");
  if (authoredSurfaceId) attrs.push(`data-surface-id="${escapeAttr(authoredSurfaceId)}"`);
  const surfaceKind = staticTextValue({ value: surface?.surfaceKind }, "value");
  if (surfaceKind) attrs.push(`data-surface-kind="${escapeAttr(surfaceKind)}"`);
  const routeKey = staticTextValue(props, "routeKey");
  if (routeKey) attrs.push(`data-surface-route-key="${escapeAttr(routeKey)}"`);
  const parentSurfaceId = staticTextValue({ value: surface?.parentId }, "value");
  if (parentSurfaceId) attrs.push(`data-surface-parent-id="${escapeAttr(parentSurfaceId)}"`);
  const domId = surfaceDomId(surface, { requireRuntimeAttachment: true });
  if (domId) attrs.push(`id="${escapeAttr(domId)}"`);
  const authoredDomId = surfaceDomId(surface);
  if (authoredDomId) attrs.push(`data-surface-dom-id="${escapeAttr(authoredDomId)}"`);
  const presentationAnchor = staticTextValue(props, "presentationAnchor");
  if (presentationAnchor) attrs.push(`data-surface-anchor="${escapeAttr(presentationAnchor)}"`);
