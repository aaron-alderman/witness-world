/**
 * mill-force-kernels.js — THE FIRST SYMMETRY-BREAK KERNELS.
 *
 * The mill-force model is faithfully expressible in the IR *except* three
 * quantities where the faithful (Excel) and grounded (first-principles) models
 * diverge by irreducible numerical *method* — not by a parameter:
 *
 *   fill_angle(J, …, method)        grid + binary-search   vs  Brent root-find
 *   gravity_area(seg, …, method)    piecewise polygon+shoelace vs 64-pt Gauss-Legendre
 *   cf_mass_moment(seg, …, method)  radial-slice centroid  vs  quadrature moment
 *
 * These are the "go deeper" leaves: lowered, named, referenced by the model, and
 * injected into the generic evaluator the same way as the (honest) std-libs. This
 * is the first compute capability — destined to be plugin.compute. Direct ports of
 * example-ports/engentus/js/mill_force_model.js.
 *
 * Everything else about mill-force (omega, charge density, shoulder angle, the
 * fill chord, per-segment angles, the whole force assembly Fw/Fc/F/resultant)
 * stays HONEST in the IR — see examples_rvm/engentus/app/models/mill-force.rvm.
 */

const TWO_PI = 2 * Math.PI;

// 64-point Gauss-Legendre nodes/weights on [-1,1] (DLMF / Abramowitz & Stegun 25.4)
const _GL64_X = [
...
function brentq(f, a, b, tol = 1e-10, maxIter = 100) {
  ...
  for (let i = 0; i < maxIter; i++) {
    ...
    if (Math.abs(e) >= tol1 && Math.abs(fa) > Math.abs(fb)) {
      let p, q, r, s2 = fb / fa;
      if (a === c) { p = 2 * xm * s2; q = 1 - s2; }
      else { q = fa / fc; r = fb / fc; p = s2 * (2 * xm * q * (q - r) - (b - a) * (r - 1)); q = (q - 1) * (r - 1) * (s2 - 1); }
      if (p > 0) q = -q; else p = -p;
...
function shoelaceArea(xs, ys) {
  let s = 0;
  for (let i = 0; i < xs.length; i++) { const j = (i + 1) % xs.length; s += xs[i] * ys[j] - xs[j] * ys[i]; }
  return Math.abs(s) / 2;
}
function shoelaceCentroid(xs, ys) {
  ...
  for (let i = 0; i < xs.length; i++) {
    ...
    area6 += cross; cx += (xs[i] + xs[j]) * cross; cy += (ys[i] + ys[j]) * cross;
  }
...
function generateRadialSlice(t1, t2, r, m, c) {
  let r1 = Math.max(0, Math.min(r, chordRadialIntercept(t1, m, c)));
  let r2 = Math.max(0, Math.min(r, chordRadialIntercept(t2, m, c)));
...
export const millForceKernels = {
  ...
  gravity_area: (t1, t2, rInner, radius, mFill, cFill, dTheta, phi, phiPrime, method) => {
    ...
    // faithful: hollow annulus + vertical slice + fully-submerged segment piece
    ...
    let A_vert = 0;
    if (inVert) { const vs = generateVerticalSlice(t1, t2, rInner, mFill, cFill); A_vert = shoelaceArea(vs.xs, vs.ys); }
    const A_seg_v = fullySub ? segmentArea(dTheta, rInner) : 0;
    return A_hollow + A_vert + A_seg_v;
  },

  // per-segment centrifugal mass-moment M_cf, s.t. Fc_r = ω²·(M_cf + radius·m_liner)
  cf_mass_moment: (t1, t2, rInner, radius, mFill, cFill, dTheta, phi, phiPrime, depth, rhoCharge, method) => {
    ...
    if (method !== "faithful") {
      ...
      // grounded: Cr·m_charge = (Mr/A)·(A·scale) = Mr·scale
      return gaussLegendreQuad(stripMoment, t2, t1) * massScale;
    }
    // faithful: radial-slice centroid·mass + fully-submerged segment piece
    const inCharge = t1 >= phiPrime;
    const fullySub = phiPrime + dTheta < t1 && t1 <= phi;
    let m_rad = 0, Cr = 0;
    if (inCharge) {
