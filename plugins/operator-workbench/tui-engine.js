...
import { declareBackendHost, declareFrontendHost, resolveServerRunner } from "../../src/host.js";
import {
  buildOperatorWorkbenchDefinition,
  buildOperatorWorkbenchDefinitionFromSpecs,
  collectAuthoredOperatorWorkbenchSpecs
} from "./operator-screen-specs.js";
import { DEFAULT_OPERATOR_WORKBENCH_DISPLAY_SETTINGS } from "./workbench/settings.js";
import {
  createOperatorWorkbenchAuthoringStore,
  resolveOperatorWorkbenchAuthoringFile
} from "./workbench/authoring.js";
import { resolveRuntimeOperatorPaths } from "../../src/runtime-operator-contract.js";
import { resolveCliRuntimeProfile } from "../../src/runtime-local-launcher.js";
import { buildPlatformModel } from "../platform/platform-model.js";
import { worldGraphProjection } from "../inspect/world-graph.js";
import {
...
const CANONICAL_ROOT_BY_ID = new Map(OPERATOR_CANONICAL_ROOTS.map(entry => [entry.id, entry]));
const CANONICAL_DOMAIN_BY_ID = new Map(OPERATOR_CANONICAL_DOMAINS.map(entry => [entry.id, entry]));
const CANONICAL_ROOT_CONTAINER_IDS = OPERATOR_CANONICAL_ROOTS.map(entry => entry.id);
const CANONICAL_BROWSE_ROOT_IDS = CANONICAL_ROOT_CONTAINER_IDS.filter(id => id !== "workbench");
const CANONICAL_DOMAIN_IDS_BY_ROOT = new Map();
for (const domain of OPERATOR_CANONICAL_DOMAINS) {
  if (!CANONICAL_DOMAIN_IDS_BY_ROOT.has(domain.rootId)) CANONICAL_DOMAIN_IDS_BY_ROOT.set(domain.rootId, []);
  CANONICAL_DOMAIN_IDS_BY_ROOT.get(domain.rootId).push(domain.id);
}
const CANONICAL_DOMAIN_MAPPING = new Map(
  OPERATOR_LEGACY_BROWSE_GROUP_MAPPINGS.map(entry => [`${entry.projection}:${entry.id}`, entry.targetDomainId])
);

const WORKBENCH_SECTION_ORDER = Object.freeze([
  "session",
  "layouts",
  "viewports",
  "panels",
...
function searchRecords(state, {
  ...
  const searchableRecords = includeWorkbench && session
    ? [...records, ...workbenchRecordCatalog(state, session)]
    : records;
...
function includeWorkbenchRecordsForSearch(session) {
  return currentPathEntries(session)[0] === "workbench";
}
...
function buildSessionGroups(session) {
  ...
  return [
    ...
    },
    {
      id: "savedViews",
      label: "Saved Views",
      count: Object.keys(session.savedResultViews ?? {}).length
    },
    {
      id: "history",
      label: "History",
      count: arrayWrap(session.history).length
    },
    {
      id: "undoRedo",
      label: "Undo / Redo",
      count: arrayWrap(session.undoStack).length + arrayWrap(session.redoStack).length
    },
    {
      id: "paneState",
      label: "Pane State",
    ...
    },
    {
      ...
      count: session.uiState?.displaySettings ? Object.keys(session.uiState.displaySettings).length || 1 : 1
    },
    {
      id: "sourceView",
      label: "Source View",
      count: session.sourceView ? 1 : 0
    },
    {
      id: "provenanceView",
      label: "Provenance View",
      count: session.provenanceView ? 1 : 0
    }
  ];
}

function workbenchRecordId(kind, id) {
  return `workbench.${kind}.${id}`;
}

function makeWorkbenchRecord({
  kind,
  id,
  title,
  summary = "",
...
function cloneWorkbenchLayoutRoot(root = null) {
  if (!root || typeof root !== "object") return null;
  if (root.kind === "panel") {
    const panelId = optionalText(root.panelId);
    return panelId ? { kind: "panel", panelId } : null;
  }
  if (root.kind !== "split") return null;
  const first = cloneWorkbenchLayoutRoot(root.first);
  const second = cloneWorkbenchLayoutRoot(root.second);
  if (!first || !second) return null;
  return {
    kind: "split",
    id: optionalText(root.id) ?? null,
    axis: optionalText(root.axis) === "horizontal" ? "horizontal" : "vertical",
    weight: Math.min(85, Math.max(15, Math.round(Number(root.weight ?? 50) || 50))),
    handle: optionalText(root.handle) ?? null,
    first,
    second
  };
}

function cloneWorkbenchPanelState(panel = null, fallbackId = null) {
  if (!panel || typeof panel !== "object") return null;
  const id = optionalText(panel.id ?? fallbackId);
  if (!id) return null;
  return {
    id,
    title: optionalText(panel.title) ?? id,
    contentKind: optionalText(panel.contentKind) ?? null,
    screenId: optionalText(panel.screenId) ?? null,
    leftScreenId: optionalText(panel.leftScreenId) ?? null,
    primaryActionId: optionalText(panel.primaryActionId) ?? null,
    secondaryMenuId: optionalText(panel.secondaryMenuId) ?? null
  };
}

function cloneWorkbenchLayoutRecord(record = null, fallbackName = null) {
  const source = record && typeof record === "object" ? record : {};
  const name = optionalText(source.name ?? fallbackName);
  const panels = Object.fromEntries(
    Object.entries(source.panels ?? {})
      .map(([panelId, panel]) => [panelId, cloneWorkbenchPanelState(panel, panelId)])
      .filter(([, panel]) => Boolean(panel))
  );
  return {
    name,
    viewportId: optionalText(source.viewportId) ?? null,
    focusedPanelId: optionalText(source.focusedPanelId) ?? null,
    root: cloneWorkbenchLayoutRoot(source.root),
    panels,
    savedAt: optionalText(source.savedAt) ?? null
  };
}

function cloneWorkbenchKeymapRecord(record = null, fallbackName = null) {
  const source = record && typeof record === "object" ? record : {};
  return {
    name: optionalText(source.name ?? fallbackName) ?? fallbackName ?? null,
    bindings: Object.fromEntries(
      Object.entries(source.bindings ?? {})
        .map(([trigger, binding]) => {
          const target = optionalText(binding?.target ?? binding?.actionId);
          if (!target) return [trigger, null];
          return [trigger, {
            target,
            targetKind: optionalText(binding?.targetKind) ?? "action"
          }];
        })
        .filter(([, binding]) => Boolean(binding))
    ),
    panelPrimaryActions: Object.fromEntries(
      Object.entries(source.panelPrimaryActions ?? {})
        .map(([panelId, actionId]) => [panelId, optionalText(actionId)])
        .filter(([, actionId]) => Boolean(actionId))
    ),
    panelSecondaryMenus: Object.fromEntries(
      Object.entries(source.panelSecondaryMenus ?? {})
        .map(([panelId, menuId]) => [panelId, optionalText(menuId)])
...
function cloneWorkbenchLayoutCatalog(layouts = {}) {
  return Object.fromEntries(
    Object.entries(layouts ?? {})
      .map(([name, record]) => [name, cloneWorkbenchLayoutRecord(record, name)])
      .filter(([, record]) => Boolean(record))
  );
}

function cloneWorkbenchKeymapCatalog(keymaps = {}) {
  return Object.fromEntries(
    Object.entries(keymaps ?? {})
      .map(([name, record]) => [name, cloneWorkbenchKeymapRecord(record, name)])
      .filter(([, record]) => Boolean(record))
  );
}

function cloneWorkbenchSpecCatalog(rows = []) {
  return Object.fromEntries(
    arrayWrap(rows)
      .map(row => [row?.id, deepClone(row)])
      .filter(([id, row]) => Boolean(optionalText(id)) && Boolean(row))
  );
}

function rebuildOperatorWorkbenchDefinitionState(state) {
  if (!state?.workbenchAuthoringSpecs) return;
  state.workbenchDefinitionOverride = buildOperatorWorkbenchDefinitionFromSpecs(state.workbenchAuthoringSpecs);
}

function setOperatorWorkbenchAuthoringSpecs(state, specs) {
  state.workbenchAuthoringSpecs = deepClone(specs);
  rebuildOperatorWorkbenchDefinitionState(state);
}

function countWorkbenchLayoutSplits(root = null) {
  if (!root || typeof root !== "object") return 0;
  if (root.kind !== "split") return 0;
  return 1 + countWorkbenchLayoutSplits(root.first) + countWorkbenchLayoutSplits(root.second);
}

function layoutTreeSummary(root = null) {
  if (!root || typeof root !== "object") return "(none)";
  if (root.kind === "panel") return optionalText(root.panelId) ?? "(panel)";
  const axis = optionalText(root.axis) === "horizontal" ? "horizontal" : "vertical";
  const first = layoutTreeSummary(root.first);
  const second = layoutTreeSummary(root.second);
  return `${axis}(${first}, ${second})`;
}

function workbenchAuthoringSourceHints(state, section = null) {
  const file = resolveOperatorWorkbenchAuthoringFile(state?.runtimeContext?.appProject ?? null);
  if (!file) return [];
  return [{
    file,
    line: null,
...
function hasWorkbenchLayoutOverlay(session) {
  return Boolean(session?.workbenchLayoutOverlay);
}

function hasWorkbenchKeymapOverlay(session) {
  return Boolean(
    Object.keys(session?.workbenchBindingOverrides ?? {}).length
    || Object.keys(session?.workbenchPanelPrimaryActionOverrides ?? {}).length
    || Object.keys(session?.workbenchPanelSecondaryMenuOverrides ?? {}).length
  );
}

function buildDefaultWorkbenchPanelStateMap(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  if ((definition.windows ?? []).length) {
    const panels = {};
    for (const panel of definition.panels ?? []) {
      const contentKind = definition.leftScreensById?.has(panel.id)
        ? "left-screen"
        : "screen";
      panels[panel.id] = {
        id: panel.id,
        ...
        contentKind,
        screenId: contentKind === "screen" ? panel.id : null,
        leftScreenId: contentKind === "left-screen" ? panel.id : null,
  ...
  const activeViewportId = definition.defaultViewport ?? definition.viewports?.[0]?.id ?? null;
  const activeViewport = activeViewportId ? (definition.viewportsById?.get(activeViewportId) ?? null) : null;
  const leftPanelId = optionalText(activeViewport?.leftScreenId) ?? "navigation";
  const rightPanelId = optionalText(activeViewport?.screenId) ?? "inspect";
  return {
    [leftPanelId]: {
      id: leftPanelId,
      title: activeViewport?.leftScreenId ? leftPanelId : "Navigation",
      contentKind: "left-screen",
      screenId: null,
      leftScreenId: activeViewport?.leftScreenId ?? null,
      primaryActionId: null,
      secondaryMenuId: null
    },
    [rightPanelId]: {
      id: rightPanelId,
      title: activeViewport?.screenId ? rightPanelId : "Inspect",
      contentKind: "screen",
      screenId: activeViewport?.screenId ?? "inspect",
      leftScreenId: null,
      primaryActionId: null,
      secondaryMenuId: null
    }
...
function buildDefaultWorkbenchLayoutRootFromSplit(splitId, definition, panelStates) {
  if (!splitId) return null;
  const split = definition.splitsById?.get(splitId) ?? null;
  if (!split) return null;
  const first = definition.splitsById?.has(split.first)
    ? buildDefaultWorkbenchLayoutRootFromSplit(split.first, definition, panelStates)
    : (panelStates[split.first] ? { kind: "panel", panelId: split.first } : null);
  const second = definition.splitsById?.has(split.second)
    ? buildDefaultWorkbenchLayoutRootFromSplit(split.second, definition, panelStates)
    : (panelStates[split.second] ? { kind: "panel", panelId: split.second } : null);
  if (!first || !second) return null;
  return {
    kind: "split",
    id: split.id,
    axis: split.axis === "horizontal" ? "horizontal" : "vertical",
    weight: Math.min(85, Math.max(15, Math.round(Number(split.firstWeight ?? 50) || 50))),
    handle: optionalText(split.handle) ?? null,
    first,
    second
  };
}

function buildDefaultWorkbenchLayoutRecord(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  const panels = buildDefaultWorkbenchPanelStateMap(state);
  if ((definition.windows ?? []).length) {
    const defaultWindowId = definition.defaultViewport && definition.windowsById?.has(definition.defaultViewport)
      ? definition.defaultViewport
      : (definition.windows?.[0]?.id ?? null);
    const windowSpec = defaultWindowId ? (definition.windowsById?.get(defaultWindowId) ?? null) : null;
    const root = buildDefaultWorkbenchLayoutRootFromSplit(windowSpec?.rootSplitId ?? null, definition, panels);
    return {
      name: "__default__",
      viewportId: defaultWindowId ?? definition.defaultViewport ?? null,
      focusedPanelId: optionalText(windowSpec?.leftPanelId ?? windowSpec?.rightPanelId) ?? Object.keys(panels)[0] ?? null,
      root,
      panels,
      savedAt: null
    };
  ...
  const activeViewportId = definition.defaultViewport ?? definition.viewports?.[0]?.id ?? null;
  const activeViewport = activeViewportId ? (definition.viewportsById?.get(activeViewportId) ?? null) : null;
  const leftPanelId = Object.values(panels).find(panel => panel.contentKind === "left-screen")?.id ?? "navigation";
  const rightPanelId = Object.values(panels).find(panel => panel.contentKind === "screen")?.id ?? "inspect";
  return {
    name: "__default__",
    ...
    root: {
      kind: "split",
      axis: activeViewport?.splitOrientation === "vertical" ? "horizontal" : "vertical",
      weight: Math.min(85, Math.max(15, Math.round(Number(activeViewport?.leftWeight ?? 28) || 28))),
      first: { kind: "panel", panelId: leftPanelId },
      second: { kind: "panel", panelId: rightPanelId }
    },
    panels,
    savedAt: null
  };
...
function currentWorkbenchLayoutRecord(state, session) {
  const defaultLayout = buildDefaultWorkbenchLayoutRecord(state);
  const savedLayout = session.activeWorkbenchLayoutName
    ? cloneWorkbenchLayoutRecord(session.savedWorkbenchLayouts?.[session.activeWorkbenchLayoutName] ?? null, session.activeWorkbenchLayoutName)
    : null;
  const overlay = cloneWorkbenchLayoutRecord(session.workbenchLayoutOverlay ?? null, session.activeWorkbenchLayoutName ?? "__overlay__");
  const base = overlay ?? savedLayout ?? defaultLayout;
  const panels = {
  ...
  return {
    name: session.activeWorkbenchLayoutName ?? null,
    viewportId: overlay?.viewportId ?? savedLayout?.viewportId ?? defaultLayout.viewportId ?? null,
    focusedPanelId: optionalText(session.workbenchFocusedPanelId)
      ?? overlay?.focusedPanelId
      ?? savedLayout?.focusedPanelId
      ?? defaultLayout.focusedPanelId
      ?? Object.keys(panels)[0]
      ?? null,
    root: cloneWorkbenchLayoutRoot(base?.root ?? defaultLayout.root),
    panels,
    savedAt: overlay?.savedAt ?? savedLayout?.savedAt ?? null
  };
}

function currentWorkbenchKeymapRecord(session) {
  const savedKeymap = session.activeWorkbenchKeymapName
    ? cloneWorkbenchKeymapRecord(session.savedWorkbenchKeymaps?.[session.activeWorkbenchKeymapName] ?? null, session.activeWorkbenchKeymapName)
    : null;
  return {
    name: session.activeWorkbenchKeymapName ?? null,
    bindings: {
      ...(savedKeymap?.bindings ?? {}),
      ...Object.fromEntries(
        Object.entries(session.workbenchBindingOverrides ?? {})
          .map(([trigger, target]) => {
            const normalizedTarget = optionalText(target);
            return normalizedTarget ? [trigger, { target: normalizedTarget, targetKind: "action" }] : [trigger, null];
          })
          .filter(([, binding]) => Boolean(binding))
      )
    },
    panelPrimaryActions: {
      ...(savedKeymap?.panelPrimaryActions ?? {}),
      ...(session.workbenchPanelPrimaryActionOverrides ?? {})
    },
    panelSecondaryMenus: {
      ...(savedKeymap?.panelSecondaryMenus ?? {}),
      ...(session.workbenchPanelSecondaryMenuOverrides ?? {})
    },
    savedAt: savedKeymap?.savedAt ?? null
  };
}

function workbenchLayoutRecords(state, session) {
  const current = currentWorkbenchLayoutRecord(state, session);
  const currentPanelCount = Object.keys(current.panels ?? {}).length;
  const currentSplitCount = countWorkbenchLayoutSplits(current.root);
  const rows = [];
  rows.push(makeWorkbenchRecord({
    kind: "layout",
    id: "current",
    title: current.name ? `Current Layout (${current.name})` : "Current Layout",
    summary: [
      current.viewportId ? `viewport=${current.viewportId}` : null,
      current.focusedPanelId ? `focus=${current.focusedPanelId}` : null,
      hasWorkbenchLayoutOverlay(session) ? "unsaved edits" : "authored/default"
    ].filter(Boolean).join(" | "),
    metadata: {
      ...current,
      recordOrigin: current.name
        ? (hasWorkbenchLayoutOverlay(session) ? "current authored overlay" : "current authored")
        : (hasWorkbenchLayoutOverlay(session) ? "current default overlay" : "current default"),
      active: true,
      current: true,
      authoredName: current.name ?? null,
      unsaved: hasWorkbenchLayoutOverlay(session),
      panelCount: currentPanelCount,
      splitCount: currentSplitCount,
      treeSummary: layoutTreeSummary(current.root),
      primaryActionLabel: "inspect"
    },
    sourceHints: workbenchAuthoringSourceHints(state, "operator_layout")
  }));
  for (const [name, layout] of Object.entries(cloneWorkbenchLayoutCatalog(session.savedWorkbenchLayouts ?? {}))
    .sort(([left], [right]) => left.localeCompare(right))) {
    const panelCount = Object.keys(layout.panels ?? {}).length;
    const splitCount = countWorkbenchLayoutSplits(layout.root);
    const active = session.activeWorkbenchLayoutName === name;
    rows.push(makeWorkbenchRecord({
      kind: "layout",
      id: name,
      title: name,
      summary: [
        layout.viewportId ? `viewport=${layout.viewportId}` : null,
        layout.focusedPanelId ? `focus=${layout.focusedPanelId}` : null,
        layout.savedAt ? `saved=${layout.savedAt}` : null
      ].filter(Boolean).join(" | "),
      metadata: {
        ...layout,
        recordOrigin: "authored",
        active,
        current: false,
        authoredName: name,
        unsaved: active && hasWorkbenchLayoutOverlay(session),
        panelCount,
        splitCount,
        treeSummary: layoutTreeSummary(layout.root),
        primaryCommand: `layout open ${name}`,
        primaryActionLabel: "open",
        primaryActionId: "workbench.layout.open"
      },
      sourceHints: workbenchAuthoringSourceHints(state, "operator_layout")
    }));
  }
...
function workbenchViewportRecords(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  return arrayWrap(definition.viewports).map(viewport =>
    makeWorkbenchRecord({
      kind: "viewport",
      id: viewport.id,
      title: viewport.title ?? viewport.id,
      summary: [
        viewport.screenId ? `screen=${viewport.screenId}` : null,
        viewport.leftScreenId ? `left=${viewport.leftScreenId}` : null,
        viewport.theme ? `theme=${viewport.theme}` : null
      ].filter(Boolean).join(" | "),
      metadata: {
        viewportId: viewport.id,
        screenId: viewport.screenId ?? null,
        leftScreenId: viewport.leftScreenId ?? null,
        theme: viewport.theme ?? null,
        layout: {
          width: viewport.width ?? null,
          height: viewport.height ?? null,
          top: viewport.top ?? null,
          bottom: viewport.bottom ?? null,
          leftWeight: viewport.leftWeight ?? null,
          rightWeight: viewport.rightWeight ?? null
        },
        authoredName: viewport.id,
        recordOrigin: viewport.origin ?? "authored"
      }
      ,
      sourceHints: workbenchAuthoringSourceHints(state, "operator_viewport")
    })
  );
}

function workbenchPanelRecords(state, session) {
  const definition = operatorWorkbenchDefinitionForState(state);
  const activeLayout = currentWorkbenchLayoutRecord(state, session);
  const activeWindowId = activeLayout.viewportId ?? definition.defaultViewport ?? null;
  const activeWindow = activeWindowId ? (definition.windowsById?.get(activeWindowId) ?? null) : null;
  return arrayWrap(definition.panels).map(panel => {
    const usedByActiveLayout = Boolean(activeLayout.panels?.[panel.id]);
    const usedByActiveWindow = Boolean(
      activeWindow
      && (activeWindow.leftPanelId === panel.id || activeWindow.rightPanelId === panel.id)
    );
    return makeWorkbenchRecord({
      kind: "panel",
      id: panel.id,
      title: panel.title ?? panel.id,
      summary: [
        `role=${panel.role ?? "aux"}`,
        panel.contentId ? `content=${panel.contentId}` : null,
        usedByActiveLayout ? "active layout" : null
      ].filter(Boolean).join(" | "),
      metadata: {
        panelId: panel.id,
        authoredName: panel.id,
        title: panel.title ?? panel.id,
        subtitle: panel.subtitle ?? null,
        role: panel.role ?? "aux",
        contentId: panel.contentId ?? null,
        shortcut: panel.shortcut ?? null,
        usedByActiveLayout,
        usedByActiveWindow,
        recordOrigin: panel.origin ?? "authored"
      },
      sourceHints: workbenchAuthoringSourceHints(state, "operator_panel")
    });
  });
}

function workbenchContentRecords(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  return arrayWrap(definition.contents).map(content => {
    const panelIds = arrayWrap(definition.panels).filter(panel => panel.contentId === content.id).map(panel => panel.id);
    return makeWorkbenchRecord({
      kind: "content",
      id: content.id,
      title: content.title ?? content.id,
      summary: [
        `kind=${content.kind ?? "detail"}`,
        content.surfaceKind ? `surface=${content.surfaceKind}` : null,
        panelIds.length ? `panels=${panelIds.join(",")}` : null
      ].filter(Boolean).join(" | "),
      metadata: {
        contentId: content.id,
        authoredName: content.id,
        title: content.title ?? content.id,
        subtitle: content.subtitle ?? null,
        kind: content.kind ?? "detail",
        screenShape: content.screenShape ?? null,
        surfaceKind: content.surfaceKind ?? null,
        datasetId: content.datasetId ?? null,
        dataSource: content.dataSource ?? null,
        leftPanelId: content.leftPanelId ?? null,
        helpText: content.helpText ?? null,
        emptyMessage: content.emptyMessage ?? null,
        rowFilterKind: content.rowFilterKind ?? null,
        rowFilterAction: content.rowFilterAction ?? null,
        shortcut: content.shortcut ?? null,
        defaultSectionId: content.defaultSectionId ?? null,
        sectionIds: structuredClone(content.sectionIds ?? []),
        columns: structuredClone(content.columns ?? []),
        collapsible: content.collapsible ?? null,
        collapsed: content.collapsed ?? null,
        priority: content.priority ?? null,
        maxPrimaryChars: content.maxPrimaryChars ?? null,
        width: content.width ?? null,
        height: content.height ?? null,
        resizable: content.resizable ?? null,
        closeIdsOnOpen: structuredClone(content.closeIdsOnOpen ?? []),
        scroll: structuredClone(content.scroll ?? []),
        panelIds,
        recordOrigin: content.origin ?? "authored"
      },
      sourceHints: workbenchAuthoringSourceHints(state, "operator_content")
    });
  });
}

function workbenchSplitRecords(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  return arrayWrap(definition.splits).map(split =>
    makeWorkbenchRecord({
      kind: "split",
      id: split.id,
      title: split.title ?? split.id,
      summary: [
        split.axis ? `axis=${split.axis}` : null,
        split.first ? `first=${split.first}` : null,
        split.second ? `second=${split.second}` : null
      ].filter(Boolean).join(" | "),
      metadata: {
        splitId: split.id,
        authoredName: split.id,
        title: split.title ?? split.id,
        axis: split.axis ?? "horizontal",
        first: split.first ?? null,
        second: split.second ?? null,
        firstWeight: split.firstWeight ?? null,
        secondWeight: split.secondWeight ?? null,
        handle: split.handle ?? null,
        recordOrigin: split.origin ?? "authored"
      },
      sourceHints: workbenchAuthoringSourceHints(state, "operator_split")
    })
  );
}

function workbenchWindowRecords(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  return arrayWrap(definition.windows).map(windowSpec =>
    makeWorkbenchRecord({
      kind: "window",
      id: windowSpec.id,
      title: windowSpec.title ?? windowSpec.id,
      summary: [
        windowSpec.rootSplitId ? `root=${windowSpec.rootSplitId}` : null,
        windowSpec.leftPanelId ? `left=${windowSpec.leftPanelId}` : null,
        windowSpec.rightPanelId ? `right=${windowSpec.rightPanelId}` : null
      ].filter(Boolean).join(" | "),
      metadata: {
        windowId: windowSpec.id,
        authoredName: windowSpec.id,
        title: windowSpec.title ?? windowSpec.id,
        theme: windowSpec.theme ?? null,
        rootSplitId: windowSpec.rootSplitId ?? null,
        leftPanelId: windowSpec.leftPanelId ?? null,
        rightPanelId: windowSpec.rightPanelId ?? null,
        topChromeId: windowSpec.topChromeId ?? null,
        bottomChromeId: windowSpec.bottomChromeId ?? null,
        topHandleId: windowSpec.topHandleId ?? null,
        bottomHandleId: windowSpec.bottomHandleId ?? null,
        width: windowSpec.width ?? null,
        height: windowSpec.height ?? null,
        top: windowSpec.top ?? null,
        bottom: windowSpec.bottom ?? null,
        overlays: structuredClone(windowSpec.overlays ?? []),
        bindings: structuredClone(windowSpec.bindings ?? []),
        recordOrigin: windowSpec.origin ?? "authored"
      },
      sourceHints: workbenchAuthoringSourceHints(state, "operator_window")
    })
  );
}

function workbenchChromeRecords(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  return arrayWrap(definition.chromes).map(chrome =>
    makeWorkbenchRecord({
      kind: "chrome",
      id: chrome.id,
      title: chrome.title ?? chrome.id,
      summary: [
        chrome.kind ? `kind=${chrome.kind}` : null,
        chrome.contentId ? `content=${chrome.contentId}` : null
      ].filter(Boolean).join(" | "),
      metadata: {
        chromeId: chrome.id,
        authoredName: chrome.id,
        title: chrome.title ?? chrome.id,
        kind: chrome.kind ?? "status_bar",
        contentId: chrome.contentId ?? null,
        recordOrigin: chrome.origin ?? "authored"
      },
      sourceHints: workbenchAuthoringSourceHints(state, "operator_chrome")
    })
  );
}

function workbenchKeymapRecords(state, session) {
  const current = currentWorkbenchKeymapRecord(session);
  const currentBindingCount = Object.keys(current.bindings ?? {}).length;
  const currentPrimaryCount = Object.keys(current.panelPrimaryActions ?? {}).length;
  const currentSecondaryCount = Object.keys(current.panelSecondaryMenus ?? {}).length;
  const rows = [
    makeWorkbenchRecord({
      kind: "keymap",
      id: "current",
      title: current.name ? `Current Keymap (${current.name})` : "Current Keymap",
      summary: `${currentBindingCount} binding${currentBindingCount === 1 ? "" : "s"}`,
      metadata: {
        ...current,
        recordOrigin: current.name
          ? (hasWorkbenchKeymapOverlay(session) ? "current authored overlay" : "current authored")
          : (hasWorkbenchKeymapOverlay(session) ? "current default overlay" : "current default"),
        active: true,
        current: true,
        authoredName: current.name ?? null,
        unsaved: hasWorkbenchKeymapOverlay(session),
        bindingCount: currentBindingCount,
        panelPrimaryOverrideCount: currentPrimaryCount,
        panelSecondaryMenuOverrideCount: currentSecondaryCount,
        primaryActionLabel: "inspect"
      },
      sourceHints: workbenchAuthoringSourceHints(state, "operator_keymap")
    })
  ];
  for (const [name, keymap] of Object.entries(cloneWorkbenchKeymapCatalog(session.savedWorkbenchKeymaps ?? {}))
    .sort(([left], [right]) => left.localeCompare(right))) {
    const bindingCount = Object.keys(keymap.bindings ?? {}).length;
    const primaryCount = Object.keys(keymap.panelPrimaryActions ?? {}).length;
    const secondaryCount = Object.keys(keymap.panelSecondaryMenus ?? {}).length;
    const active = session.activeWorkbenchKeymapName === name;
    rows.push(makeWorkbenchRecord({
      kind: "keymap",
      id: name,
      title: name,
      summary: [
        ...
        keymap.savedAt ? `saved=${keymap.savedAt}` : null
      ].filter(Boolean).join(" | "),
      metadata: {
        ...keymap,
        recordOrigin: "authored",
        ...
        current: false,
        authoredName: name,
        unsaved: active && hasWorkbenchKeymapOverlay(session),
        bindingCount,
        panelPrimaryOverrideCount: primaryCount,
        panelSecondaryMenuOverrideCount: secondaryCount,
        primaryCommand: `keymap open ${name}`,
        primaryActionLabel: "open",
        primaryActionId: "workbench.keymap.open"
      },
      sourceHints: workbenchAuthoringSourceHints(state, "operator_keymap")
    }));
  }
...
function workbenchActionRecords(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  return arrayWrap(definition.actions).map(action =>
    makeWorkbenchRecord({
      kind: "action",
      id: action.id,
      ...
      summary: [
        action.kind,
        action.builtin ? `builtin=${action.builtin}` : null,
        action.overlayId ? `overlay=${action.overlayId}` : null,
        action.screenId ? `screen=${action.screenId}` : null
      ].filter(Boolean).join(" | "),
      metadata: {
        actionId: action.id,
        kind: action.kind,
        builtin: action.builtin ?? null,
        overlayId: action.overlayId ?? null,
        screenId: action.screenId ?? null,
...
function workbenchMenuRecords(state) {
  const definition = operatorWorkbenchDefinitionForState(state);
  return arrayWrap(definition.menus).map(menu =>
    makeWorkbenchRecord({
      kind: "menu",
      id: menu.id,
      title: menu.title ?? menu.id,
      summary: `${arrayWrap(menu.itemActionIds).length} action${arrayWrap(menu.itemActionIds).length === 1 ? "" : "s"}`,
      metadata: {
        menuId: menu.id,
        itemActionIds: structuredClone(menu.itemActionIds ?? [])
      }
    })
  );
}

const SYNTHETIC_WORKBENCH_ACTION_SPECS = Object.freeze([
  { id: "workbench.layout.new", title: "New Layout", kind: "builtin", builtin: "workbench-layout-new" },
  { id: "workbench.layout.duplicate", title: "Duplicate Layout", kind: "builtin", builtin: "workbench-layout-duplicate" },
  { id: "workbench.layout.rename", title: "Rename Layout", kind: "builtin", builtin: "workbench-layout-rename" },
  { id: "workbench.layout.delete", title: "Delete Layout", kind: "builtin", builtin: "workbench-layout-delete" },
  { id: "workbench.layout.open", title: "Open Layout", kind: "builtin", builtin: "workbench-layout-open" },
  { id: "workbench.layout.save", title: "Save Layout", kind: "builtin", builtin: "workbench-layout-save" },
  { id: "workbench.layout.reset", title: "Reset Layout", kind: "builtin", builtin: "workbench-layout-reset" },
  { id: "workbench.keymap.new", title: "New Keymap", kind: "builtin", builtin: "workbench-keymap-new" },
  { id: "workbench.keymap.duplicate", title: "Duplicate Keymap", kind: "builtin", builtin: "workbench-keymap-duplicate" },
  { id: "workbench.keymap.rename", title: "Rename Keymap", kind: "builtin", builtin: "workbench-keymap-rename" },
  { id: "workbench.keymap.delete", title: "Delete Keymap", kind: "builtin", builtin: "workbench-keymap-delete" },
  { id: "workbench.keymap.open", title: "Open Keymap", kind: "builtin", builtin: "workbench-keymap-open" },
  { id: "workbench.keymap.save", title: "Save Keymap", kind: "builtin", builtin: "workbench-keymap-save" },
  { id: "workbench.keymap.reset", title: "Reset Keymap", kind: "builtin", builtin: "workbench-keymap-reset" },
  { id: "workbench.panel.open", title: "Open Panel", kind: "builtin", builtin: "workbench-panel-open" },
  { id: "workbench.panel.duplicate", title: "Duplicate Panel", kind: "builtin", builtin: "workbench-panel-duplicate" },
  { id: "workbench.panel.rename", title: "Rename Panel", kind: "builtin", builtin: "workbench-panel-rename" },
  { id: "workbench.panel.delete", title: "Delete Panel", kind: "builtin", builtin: "workbench-panel-delete" },
  { id: "workbench.panel.assign-content", title: "Assign Panel Content", kind: "builtin", builtin: "workbench-panel-assign-content" },
  { id: "workbench.content.open", title: "Open Content", kind: "builtin", builtin: "workbench-content-open" },
  { id: "workbench.content.duplicate", title: "Duplicate Content", kind: "builtin", builtin: "workbench-content-duplicate" },
  { id: "workbench.content.rename", title: "Rename Content", kind: "builtin", builtin: "workbench-content-rename" },
  { id: "workbench.content.delete", title: "Delete Content", kind: "builtin", builtin: "workbench-content-delete" }
]);

function workbenchRecordCatalog(state, session) {
  return [
    ...workbenchLayoutRecords(state, session),
    ...workbenchViewportRecords(state),
    ...workbenchPanelRecords(state, session),
    ...workbenchContentRecords(state),
    ...workbenchKeymapRecords(state, session),
    ...workbenchActionRecords(state),
    ...workbenchMenuRecords(state),
    ...workbenchSplitRecords(state),
    ...workbenchWindowRecords(state),
    ...workbenchChromeRecords(state)
  ];
}

function findWorkbenchRecord(state, session, token) {
  const trimmed = optionalText(token);
  if (!trimmed) return null;
  const normalized = normalizeSearchText(trimmed);
  return workbenchRecordCatalog(state, session).find(record =>
    record.id === trimmed
    || recordDescriptor(record) === trimmed
    || normalizeSearchText(record.id) === normalized
    || normalizeSearchText(record.title) === normalized
  ) ?? null;
}

function buildWorkbenchGroups(state, session) {
  return [
    {
      id: "session",
      label: "Session",
      count: buildSessionGroups(session).length,
      summary: "Selection, focus, aliases, notes, preview session, and mini-programs."
    },
    {
      id: "layouts",
      label: "Layouts",
      count: workbenchLayoutRecords(state, session).length,
      summary: "Named workbench layouts plus the current session-local split tree."
    },
    {
      id: "viewports",
      label: "Viewports",
      count: workbenchViewportRecords(state).length,
      summary: "Authored viewport seeds and active viewport identity."
    },
    {
      id: "panels",
      label: "Panels",
      count: workbenchPanelRecords(state, session).length,
      summary: "Authored panel records and their current content assignments."
    },
    {
      id: "contents",
      label: "Contents",
      count: workbenchContentRecords(state).length,
      summary: "Authored panel content records and panel bindings."
    },
    {
      id: "keymaps",
      label: "Keymaps",
      count: workbenchKeymapRecords(state, session).length,
      summary: "Named semantic keymaps plus the current binding overlay."
    },
    {
      id: "actions",
      label: "Actions",
      count: workbenchActionRecords(state).length,
      summary: "Semantic actions available to panels, menus, and bindings."
    },
    {
      id: "menus",
      label: "Menus",
      count: workbenchMenuRecords(state).length,
      summary: "Context and secondary-action menus available in the workbench."
    },
    {
      id: "splits",
      label: "Splits",
      count: workbenchSplitRecords(state).length,
      summary: "Authored split graph nodes used by workbench windows."
    },
    {
      id: "windows",
      label: "Windows",
      count: workbenchWindowRecords(state).length,
      summary: "Authored workbench window shells and their panel bindings."
    },
    {
      id: "chromes",
      label: "Chromes",
      count: workbenchChromeRecords(state).length,
      summary: "Authored top and bottom chrome surfaces."
    }
  ];
}

function recordCanonicalDomainId(record) {
  const projection = record?.scope === "platform" ? "platform" : "world";
  return CANONICAL_DOMAIN_MAPPING.get(`${projection}:${record?.kind ?? ""}`) ?? null;
}

function buildCanonicalGroupedRecords(records, rootId) {
  const allowedDomainIds = CANONICAL_DOMAIN_IDS_BY_ROOT.get(rootId) ?? [];
  const allowedSet = new Set(allowedDomainIds);
  const grouped = new Map();
  for (const record of arrayWrap(records)) {
    const domainId = recordCanonicalDomainId(record);
    if (!domainId || !allowedSet.has(domainId)) continue;
    if (!grouped.has(domainId)) grouped.set(domainId, []);
    grouped.get(domainId).push(record);
  }
  return allowedDomainIds
    .filter(domainId => grouped.has(domainId))
    .map(domainId => {
      const domain = CANONICAL_DOMAIN_BY_ID.get(domainId);
      return {
        id: domainId,
        label: domain?.label ?? domainId,
        rootId,
        records: grouped.get(domainId)
          .slice()
          .sort((left, right) =>
            left.title.localeCompare(right.title)
            || left.id.localeCompare(right.id))
...
function buildCanonicalRootSummaries(records, rootIds = CANONICAL_BROWSE_ROOT_IDS) {
  return rootIds.map(rootId => {
    const groups = buildCanonicalGroupedRecords(records, rootId);
    return {
      rootId,
      groups,
...
function focusRootContainerId(rootId) {
  return `focus:${rootId}`;
}

function focusDomainContainerId(rootId, domainId) {
  return `focus:${rootId}:${domainId}`;
}

function parseFocusContainerId(containerId) {
  if (containerId === FOCUS_ROOT_CONTAINER_ID) return { kind: "root" };
  if (!containerId.startsWith("focus:")) return null;
  const parts = containerId.split(":");
  if (parts.length === 2) return { kind: "canonical-root", rootId: parts[1] };
  if (parts.length === 3) return { kind: "canonical-domain", rootId: parts[1], domainId: parts[2] };
  return null;
}
...
function createContainerIndex(state, session) {
  ...
  if (isContextFocusActive(session)) {
    ...
    const focusRoots = buildCanonicalRootSummaries(focusedRecords).filter(root => root.count > 0);
    add(makeContainer({
    ...
    for (const root of focusRoots) {
      add(makeContainer({
        id: focusRootContainerId(root.rootId),
        name: root.rootId,
        label: CANONICAL_ROOT_BY_ID.get(root.rootId)?.label ?? root.rootId,
        summary: `${root.count} item${root.count === 1 ? "" : "s"}.`,
        parentId: FOCUS_ROOT_CONTAINER_ID
      }));
      for (const group of root.groups) {
        add(makeContainer({
          id: focusDomainContainerId(root.rootId, group.id),
          name: group.id,
          label: group.label,
          summary: `${group.records.length} item${group.records.length === 1 ? "" : "s"}.`,
          parentId: focusRootContainerId(root.rootId)
        }));
      }
    }
    return index;
  ...
  add(makeContainer({
    ...
    summary: "Workbench state plus canonical operator ontology roots.",
    parentId: null
  }));

  for (const rootId of CANONICAL_ROOT_CONTAINER_IDS) {
    const root = CANONICAL_ROOT_BY_ID.get(rootId);
    add(makeContainer({
      id: rootId,
      name: rootId,
  ...
  for (const section of WORKBENCH_SECTION_ORDER) {
    const group = buildWorkbenchGroups(state, session).find(entry => entry.id === section);
    add(makeContainer({
      id: `workbench:${section}`,
      name: section,
      label: group.label,
      summary: group.summary ?? `${group.count} item${group.count === 1 ? "" : "s"}.`,
      parentId: "workbench"
    }));
  }
  ...
  for (const section of SESSION_SECTION_ORDER) {
    ...
    add(makeContainer({
      id: `workbench:session:${section}`,
      name: section,
      label: group?.label ?? section,
      summary: group ? `${group.count} item${group.count === 1 ? "" : "s"}.` : "",
      parentId: "workbench:session"
    }));
  }

  const canonicalGroups = new Map();
  for (const rootId of CANONICAL_BROWSE_ROOT_IDS) {
    for (const group of buildCanonicalGroupedRecords([...state.worldRecords, ...state.platformRecords], rootId)) {
      canonicalGroups.set(`${rootId}:${group.id}`, group);
      add(makeContainer({
        id: `${rootId}:${group.id}`,
        name: group.id,
        label: group.label,
...
function sessionSectionItems(state, session, sectionId) {
  if (sectionId === "workbench:session:selection") {
    if (!session.selectionId) return [];
    const record = state.recordIndex.get(session.selectionId);
    if (!record) return [];
    return [{ type: "record", record }];
  }
  if (sectionId === "workbench:session:focus") {
    if (!session.focusKind && !session.focusId) return [];
    return [{
      type: "info",
      label: session.focusLabel ?? `context:${session.focusId ?? "(none)"}`,
      detail: [
        `kind=${session.focusKind ?? "(none)"}`,
        `id=${session.focusId ?? "(none)"}`,
        `return=${arrayWrap(session.focusReturnPath).length ? arrayWrap(session.focusReturnPath).join("/") : "root"}`
      ].join(" | ")
    }];
  }
  if (sectionId === "workbench:session:aliases") {
  ...
  if (sectionId === "workbench:session:preview") {
  ...
  if (sectionId === "workbench:session:notes") {
    return session.notes.map(entry => ({ type: "note", entry }));
  }
  if (sectionId === "workbench:session:programs") {
  ...
  if (sectionId === "workbench:session:savedViews") {
    return workbenchSectionItems(state, session, "workbench:savedViews");
  }
  if (sectionId === "workbench:session:history") {
    return workbenchSectionItems(state, session, "workbench:history");
  }
  if (sectionId === "workbench:session:undoRedo") {
    return workbenchSectionItems(state, session, "workbench:undoRedo");
  }
  if (sectionId === "workbench:session:paneState") {
    return workbenchSectionItems(state, session, "workbench:paneState");
  }
  if (sectionId === "workbench:session:displaySettings") {
    return workbenchSectionItems(state, session, "workbench:displaySettings");
  }
  if (sectionId === "workbench:session:sourceView") {
    return workbenchSectionItems(state, session, "workbench:sourceView");
  }
  if (sectionId === "workbench:session:provenanceView") {
    return workbenchSectionItems(state, session, "workbench:provenanceView");
  }
  return [];
}

function workbenchSectionItems(state, session, sectionId) {
  if (sectionId === "workbench:layouts") {
    return workbenchLayoutRecords(state, session).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:viewports") {
    return workbenchViewportRecords(state).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:panels") {
    return workbenchPanelRecords(state, session).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:contents") {
    return workbenchContentRecords(state).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:keymaps") {
    return workbenchKeymapRecords(state, session).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:actions") {
    return workbenchActionRecords(state).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:menus") {
    return workbenchMenuRecords(state).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:splits") {
    return workbenchSplitRecords(state).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:windows") {
    return workbenchWindowRecords(state).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:chromes") {
    return workbenchChromeRecords(state).map(record => ({ type: "record", record }));
  }
  if (sectionId === "workbench:savedViews") {
    return Object.entries(session.savedResultViews ?? {})
      .sort(([left], [right]) => left.localeCompare(right))
      .map(([name, config]) => ({
        type: "info",
        label: name,
        detail: [
          ...
          `columns=${arrayWrap(config?.columns).join(",") || "(default)"}`
        ].join(" | ")
      }));
  }
  if (sectionId === "workbench:history") {
    return arrayWrap(session.history)
      .slice()
      .reverse()
      .map(entry => ({
        type: "info",
        label: entry.command ?? "(command)",
        detail: [
          entry.status ?? "info",
          entry.at ?? "(time unknown)"
        ].join(" | ")
      }));
  }
  if (sectionId === "workbench:undoRedo") {
    return [
      {
        type: "info",
        label: "Undo Stack",
        detail: `${arrayWrap(session.undoStack).length} entr${arrayWrap(session.undoStack).length === 1 ? "y" : "ies"}`
      },
      {
        type: "info",
        label: "Redo Stack",
        detail: `${arrayWrap(session.redoStack).length} entr${arrayWrap(session.redoStack).length === 1 ? "y" : "ies"}`
      }
    ];
  }
  if (sectionId === "workbench:paneState") {
    return [{
      type: "info",
      label: "Pane State",
      detail: [
        `screen=${session.activeWorkbenchScreenId ?? "inspect"}`,
        `tab=${session.activeWorkbenchInspectorTab ?? "inspect"}`,
        `sections=${Object.keys(session.workbenchSectionStateByScreenId ?? {}).length}`
      ].join(" | ")
    }];
  }
  if (sectionId === "workbench:viewportLayout") {
    return [{
      type: "info",
      label: "Viewport Layout",
      detail: "Pane sizes, split positions, and viewport arrangement."
    }];
  }
  if (sectionId === "workbench:displaySettings") {
    const settings = session.uiState?.displaySettings ?? null;
  ...
  if (sectionId === "workbench:sourceView") {
    if (!session.sourceView) return [];
    return [{
      type: "info",
      label: session.sourceView.targetId ?? "source",
      detail: `${arrayWrap(session.sourceView.sources).length} source entr${arrayWrap(session.sourceView.sources).length === 1 ? "y" : "ies"}`
    }];
  }
  if (sectionId === "workbench:provenanceView") {
    if (!session.provenanceView) return [];
    return [{
      type: "info",
      label: session.provenanceView.targetId ?? "provenance",
      detail: `${arrayWrap(session.provenanceView.entries).length} provenance entr${arrayWrap(session.provenanceView.entries).length === 1 ? "y" : "ies"}`
    }];
  }
  return [];
}
...
function buildContainerEntries(state, session, containerId) {
  const parsedFocusContainer = isContextFocusActive(session) ? parseFocusContainerId(containerId) : null;
  if (parsedFocusContainer?.kind === "root") {
    return buildCanonicalRootSummaries(focusScopedWorldRecords(state, session.focusId))
      .filter(root => root.count > 0)
      .map(root => ({
      type: "container",
      container: state.containerIndex.get(focusRootContainerId(root.rootId)),
      count: root.count
      }));
  }
  if (parsedFocusContainer?.kind === "canonical-root") {
    const root = buildCanonicalRootSummaries(focusScopedWorldRecords(state, session.focusId))
      .filter(entry => entry.count > 0)
      .find(entry => entry.rootId === parsedFocusContainer.rootId);
    return (root?.groups ?? []).map(group => ({
      type: "container",
      container: state.containerIndex.get(focusDomainContainerId(parsedFocusContainer.rootId, group.id)),
      count: group.records.length
    }));
  }
  if (parsedFocusContainer?.kind === "canonical-domain") {
    const group = buildCanonicalGroupedRecords(
      focusScopedWorldRecords(state, session.focusId),
      parsedFocusContainer.rootId
    ).find(entry => entry.id === parsedFocusContainer.domainId);
    return (group?.records ?? []).map(record => ({ type: "record", record }));
  }
  if (containerId === ROOT_CONTAINER_ID) {
    return CANONICAL_ROOT_CONTAINER_IDS.map(id => ({
      type: "container",
      container: state.containerIndex.get(id)
    }));
  }
  if (containerId === "workbench") {
    return buildWorkbenchGroups(state, session).map(group => ({
      type: "container",
      container: state.containerIndex.get(`workbench:${group.id}`),
      count: group.count
    }));
  }
  if (containerId === "workbench:session") {
    return buildSessionGroups(session).map(group => ({
      type: "container",
      container: state.containerIndex.get(`workbench:session:${group.id}`),
      count: group.count
    }));
  }
  if (containerId.startsWith("workbench:session:")) {
    return sessionSectionItems(state, session, containerId);
  }
  if (containerId.startsWith("workbench:")) {
    return workbenchSectionItems(state, session, containerId);
  }
  if (CANONICAL_BROWSE_ROOT_IDS.includes(containerId)) {
    return buildCanonicalGroupedRecords([...state.worldRecords, ...state.platformRecords], containerId).map(group => ({
      type: "container",
      container: state.containerIndex.get(`${containerId}:${group.id}`),
      count: group.records.length
    }));
  }
  const canonicalGroupMatch = containerId.match(/^([a-zA-Z]+):([A-Za-z0-9]+)$/);
  if (canonicalGroupMatch && canonicalGroupMatch[1] !== "workbench" && canonicalGroupMatch[1] !== "focus") {
    const rootId = canonicalGroupMatch[1];
    const groupId = canonicalGroupMatch[2];
    const group = buildCanonicalGroupedRecords([...state.worldRecords, ...state.platformRecords], rootId)
...
function renderEntry(entry, index, session) {
  ...
  if (entry.type === "info") {
    return `${marker}[${index}] ${entry.label}${entry.detail ? ` - ${entry.detail}` : ""}`;
  }
...
function resolveRecordReference(state, session, token) {
  ...
  const workbenchRecord = findWorkbenchRecord(state, session, trimmed);
  if (workbenchRecord) return workbenchRecord;
  if (state.recordIndex.has(trimmed)) return state.recordIndex.get(trimmed);
  const record = [...state.recordIndex.values(), ...workbenchRecordCatalog(state, session)].find(entry => entry.title === trimmed);
  return record ?? null;
}
...
function renderWorkbenchLayoutDetails(record) {
  const metadata = record?.metadata && typeof record.metadata === "object" ? record.metadata : {};
  const splitCount = metadata.splitCount ?? countWorkbenchLayoutSplits(metadata.root);
  const panelCount = metadata.panelCount ?? Object.keys(metadata.panels ?? {}).length;
  const lines = [
    `${record.title}`,
    `id: ${record.id}`,
    `scope: workbench`,
    `kind: layout`,
    `name: ${metadata.authoredName ?? metadata.name ?? "(current default)"}`,
    `origin: ${metadata.recordOrigin ?? "authored"}`,
    `viewport id: ${metadata.viewportId ?? "(none)"}`,
    `focused panel id: ${metadata.focusedPanelId ?? "(none)"}`,
    `panel count: ${panelCount}`,
    `split-tree: ${splitCount} split${splitCount === 1 ? "" : "s"}`,
    `tree summary: ${metadata.treeSummary ?? layoutTreeSummary(metadata.root)}`,
    `active layout: ${metadata.active ? "yes" : "no"}`,
    `unsaved changes: ${metadata.unsaved ? "yes" : "no"}`
  ];
  if (metadata.current) {
    lines.push(`authored base: ${metadata.authoredName ?? "(default authored workbench)"}`);
  } else if (metadata.savedAt) {
    lines.push(`saved at: ${metadata.savedAt}`);
  }
  return lines.join("\n");
}

function renderWorkbenchKeymapDetails(record) {
  const metadata = record?.metadata && typeof record.metadata === "object" ? record.metadata : {};
  const bindingCount = metadata.bindingCount ?? Object.keys(metadata.bindings ?? {}).length;
  const primaryCount = metadata.panelPrimaryOverrideCount ?? Object.keys(metadata.panelPrimaryActions ?? {}).length;
  const secondaryCount = metadata.panelSecondaryMenuOverrideCount ?? Object.keys(metadata.panelSecondaryMenus ?? {}).length;
  const lines = [
    `${record.title}`,
    `id: ${record.id}`,
    `scope: workbench`,
    ...
    `panel primary override count: ${primaryCount}`,
    `panel secondary menu override count: ${secondaryCount}`,
    `active keymap: ${metadata.active ? "yes" : "no"}`,
    `unsaved changes: ${metadata.unsaved ? "yes" : "no"}`
  ];
  if (metadata.current) {
    lines.push(`authored base: ${metadata.authoredName ?? "(default authored workbench)"}`);
  ...
  if (bindings.length) {
    lines.push("bindings:");
    for (const [trigger, binding] of bindings.slice(0, 8)) {
      lines.push(`  - ${trigger}: ${binding?.target ?? "(none)"}`);
    }
  }
  return lines.join("\n");
}

function renderWorkbenchPanelDetails(record) {
  const metadata = record?.metadata && typeof record.metadata === "object" ? record.metadata : {};
  return [
    `${record.title}`,
    `id: ${record.id}`,
    `scope: workbench`,
    `kind: panel`,
    `name: ${metadata.authoredName ?? metadata.panelId ?? record.title}`,
    `role: ${metadata.role ?? "aux"}`,
    `content id: ${metadata.contentId ?? "(none)"}`,
    `shortcut: ${metadata.shortcut ?? "(none)"}`,
    `used by active layout: ${metadata.usedByActiveLayout ? "yes" : "no"}`,
    `used by active window: ${metadata.usedByActiveWindow ? "yes" : "no"}`
  ].join("\n");
}

function renderWorkbenchContentDetails(record) {
  const metadata = record?.metadata && typeof record.metadata === "object" ? record.metadata : {};
  const lines = [
    `${record.title}`,
    `id: ${record.id}`,
    `scope: workbench`,
    `kind: content`,
    `name: ${metadata.authoredName ?? metadata.contentId ?? record.title}`,
    `content kind: ${metadata.kind ?? "detail"}`,
    `surface kind: ${metadata.surfaceKind ?? "(none)"}`,
    `screen shape: ${metadata.screenShape ?? "(none)"}`,
    `dataset id: ${metadata.datasetId ?? "(none)"}`,
    `data source: ${metadata.dataSource ?? "(none)"}`,
    `left panel: ${metadata.leftPanelId ?? "(none)"}`,
    `default section: ${metadata.defaultSectionId ?? "(none)"}`
  ];
  if (arrayWrap(metadata.sectionIds).length) lines.push(`sections: ${arrayWrap(metadata.sectionIds).join(", ")}`);
  if (arrayWrap(metadata.panelIds).length) lines.push(`panels: ${arrayWrap(metadata.panelIds).join(", ")}`);
  return lines.join("\n");
}

function renderWorkbenchViewportDetails(record) {
  const metadata = record?.metadata && typeof record.metadata === "object" ? record.metadata : {};
  return [
    `${record.title}`,
    `id: ${record.id}`,
    `scope: workbench`,
    `kind: viewport`,
    `screen id: ${metadata.screenId ?? "(none)"}`,
    `left screen id: ${metadata.leftScreenId ?? "(none)"}`,
    `theme: ${metadata.theme ?? "(none)"}`,
    `width: ${metadata.layout?.width ?? "(none)"}`,
    `height: ${metadata.layout?.height ?? "(none)"}`,
    `top: ${metadata.layout?.top ?? "(none)"}`,
    `bottom: ${metadata.layout?.bottom ?? "(none)"}`,
    `left weight: ${metadata.layout?.leftWeight ?? "(none)"}`,
    `right weight: ${metadata.layout?.rightWeight ?? "(none)"}`
  ].join("\n");
}

function renderWorkbenchSplitDetails(record) {
  const metadata = record?.metadata && typeof record.metadata === "object" ? record.metadata : {};
  return [
    `${record.title}`,
    `id: ${record.id}`,
    `scope: workbench`,
    `kind: split`,
    `axis: ${metadata.axis ?? "horizontal"}`,
    `first: ${metadata.first ?? "(none)"}`,
    `second: ${metadata.second ?? "(none)"}`,
    `first weight: ${metadata.firstWeight ?? "(none)"}`,
    `second weight: ${metadata.secondWeight ?? "(none)"}`,
    `handle: ${metadata.handle ?? "(none)"}`
  ].join("\n");
}

function renderWorkbenchWindowDetails(record) {
  const metadata = record?.metadata && typeof record.metadata === "object" ? record.metadata : {};
  return [
    `${record.title}`,
    `id: ${record.id}`,
    ...
    `kind: window`,
    `theme: ${metadata.theme ?? "(none)"}`,
    `root split: ${metadata.rootSplitId ?? "(none)"}`,
    `left panel: ${metadata.leftPanelId ?? "(none)"}`,
    `right panel: ${metadata.rightPanelId ?? "(none)"}`,
    `top chrome: ${metadata.topChromeId ?? "(none)"}`,
    `bottom chrome: ${metadata.bottomChromeId ?? "(none)"}`,
    `width: ${metadata.width ?? "(none)"}`,
    `height: ${metadata.height ?? "(none)"}`,
    `top: ${metadata.top ?? "(none)"}`,
    `bottom: ${metadata.bottom ?? "(none)"}`
  ].join("\n");
}

function renderWorkbenchChromeDetails(record) {
  const metadata = record?.metadata && typeof record.metadata === "object" ? record.metadata : {};
  return [
    `${record.title}`,
    `id: ${record.id}`,
    `scope: workbench`,
    `kind: chrome`,
    `chrome kind: ${metadata.kind ?? "status_bar"}`,
    `content id: ${metadata.contentId ?? "(none)"}`
  ].join("\n");
}
...
function workbenchRowsFromEntries(entries, session) {
  return entries.map((entry, index) => {
    let primaryAction = workbenchPrimaryActionForType(entry.type, index);
    const row = {
      index: index + 1,
      type: entry.type,
    ...
    if (entry.type === "record") {
      ...
      row.record = structuredClone(entry.record);
      row.kind = entry.record.kind;
      row.scope = entry.record.scope;
      if (entry.record.scope === "workbench") {
        const command = optionalText(entry.record.metadata?.primaryCommand);
        const actionId = optionalText(entry.record.metadata?.primaryActionId);
        const label = optionalText(entry.record.metadata?.primaryActionLabel);
        if (command || actionId) {
          primaryAction = {
            command: command ?? null,
            label: label ?? (command ? "open" : "inspect"),
            actionId: actionId ?? null
          };
          row.primaryAction = primaryAction;
          row.actionable = true;
        }
      }
      return row;
    ...
    if (entry.type === "info") {
      row.label = optionalText(entry.label) ?? "Info";
      row.summary = optionalText(entry.detail) ?? "";
      row.actionable = false;
      row.primaryAction = null;
      return row;
    }
    row.label = "Item";
    row.summary = "";
...
export async function inspectRecordForWorkbench(state, session, record) {
  ...
  if (record.scope === "workbench" && record.kind === "layout") {
    return {
      title: record.title,
      inspectLines: renderWorkbenchLayoutDetails(record).split("\n"),
      sourceLines: record.sourceHints?.length
        ? record.sourceHints.map((source, index) => `${index === 0 ? "source:" : "      "} ${source.file}`)
        : ["Source unavailable for this workbench layout."],
      provenanceLines: ["Provenance unavailable for this workbench layout."],
      references: [],
      inspection: null,
      target: {
  ...
  if (record.scope === "workbench" && record.kind === "keymap") {
    return {
      title: record.title,
      inspectLines: renderWorkbenchKeymapDetails(record).split("\n"),
      sourceLines: record.sourceHints?.length
        ? record.sourceHints.map((source, index) => `${index === 0 ? "source:" : "      "} ${source.file}`)
        : ["Source unavailable for this workbench keymap."],
      provenanceLines: ["Provenance unavailable for this workbench keymap."],
      references: [],
      inspection: null,
      target: {
        kind: "workbench-keymap",
        id: record.id,
        label: record.title,
        uri: createLinkForRecord(state, record)?.uri ?? null,
        pinned: false,
  ...
  if (record.scope === "workbench" && record.kind === "panel") {
    return {
      title: record.title,
      inspectLines: renderWorkbenchPanelDetails(record).split("\n"),
      sourceLines: record.sourceHints?.length
        ? record.sourceHints.map((source, index) => `${index === 0 ? "source:" : "      "} ${source.file}`)
        : ["Source unavailable for this workbench panel."],
      provenanceLines: ["Provenance unavailable for this workbench panel."],
      references: [],
      inspection: null,
      target: {
  ...
  if (record.scope === "workbench" && record.kind === "content") {
    return {
      title: record.title,
      inspectLines: renderWorkbenchContentDetails(record).split("\n"),
      sourceLines: record.sourceHints?.length
        ? record.sourceHints.map((source, index) => `${index === 0 ? "source:" : "      "} ${source.file}`)
        : ["Source unavailable for this workbench content."],
      provenanceLines: ["Provenance unavailable for this workbench content."],
      references: [],
      inspection: null,
      target: {
  ...
  if (record.scope === "workbench" && record.kind === "viewport") {
    return {
      title: record.title,
      inspectLines: renderWorkbenchViewportDetails(record).split("\n"),
      sourceLines: record.sourceHints?.length
        ? record.sourceHints.map((source, index) => `${index === 0 ? "source:" : "      "} ${source.file}`)
        : ["Source unavailable for this workbench viewport."],
      provenanceLines: ["Provenance unavailable for this workbench viewport."],
      references: [],
      inspection: null,
      target: {
        kind: "workbench-viewport",
        id: record.id,
        label: record.title,
        uri: createLinkForRecord(state, record)?.uri ?? null,
        pinned: false,
  ...
  if (record.scope === "workbench" && record.kind === "split") {
    return {
      title: record.title,
      inspectLines: renderWorkbenchSplitDetails(record).split("\n"),
      sourceLines: record.sourceHints?.length
        ? record.sourceHints.map((source, index) => `${index === 0 ? "source:" : "      "} ${source.file}`)
        : ["Source unavailable for this workbench split."],
      provenanceLines: ["Provenance unavailable for this workbench split."],
      references: [],
      inspection: null,
      target: {
        kind: "workbench-split",
        id: record.id,
        label: record.title,
        uri: createLinkForRecord(state, record)?.uri ?? null,
        pinned: false,
  ...
  if (record.scope === "workbench" && record.kind === "window") {
    return {
      title: record.title,
      inspectLines: renderWorkbenchWindowDetails(record).split("\n"),
      sourceLines: record.sourceHints?.length
        ? record.sourceHints.map((source, index) => `${index === 0 ? "source:" : "      "} ${source.file}`)
        : ["Source unavailable for this workbench window."],
      provenanceLines: ["Provenance unavailable for this workbench window."],
      references: [],
      inspection: null,
      target: {
  ...
  if (record.scope === "workbench" && record.kind === "chrome") {
    return {
      title: record.title,
      inspectLines: renderWorkbenchChromeDetails(record).split("\n"),
      sourceLines: record.sourceHints?.length
        ? record.sourceHints.map((source, index) => `${index === 0 ? "source:" : "      "} ${source.file}`)
        : ["Source unavailable for this workbench chrome."],
      provenanceLines: ["Provenance unavailable for this workbench chrome."],
      references: [],
      inspection: null,
      target: {
...
function operatorWorkbenchDefinitionForState(state) {
  return state?.workbenchDefinitionOverride
    ?? state.runtimeContext?.appProject?.extensionModels?.byId?.get?.("operatorWorkbench")
    ?? buildOperatorWorkbenchDefinition(null);
}
...
function operatorWorkbenchActionSpec(state, actionId) {
  if (!actionId) return null;
  return operatorWorkbenchDefinitionForState(state).actionsById?.get(actionId)
    ?? SYNTHETIC_WORKBENCH_ACTION_SPECS.find(action => action.id === actionId)
    ?? null;
}

function operatorWorkbenchMenuSpec(state, menuId) {
  if (!menuId) return null;
  return operatorWorkbenchDefinitionForState(state).menusById?.get(menuId) ?? null;
}
...
function isLegacyDatasetPrimaryAction(action) {
  return ["open-link", "source-open", "provenance-open", "inspect-record", "none"].includes(optionalText(action));
}
...
function buildDatasetScreenRow(row, {
  ...
  const primaryCommand = isLegacyDatasetPrimaryAction(primaryAction)
    ? datasetRowPrimaryCommand(primaryAction, { ...row, baseIndex })
    : null;
  return {
    ...deepClone(row),
    baseIndex,
    primaryAction,
    primaryActionId: primaryCommand ? null : (isLegacyDatasetPrimaryAction(primaryAction) ? null : primaryAction),
    primaryCommand,
...
function leftPaneRowFromWorkbenchRow(row, index, session) {
  ...
  return {
    index: index + 1,
    type: "record",
    actionable: Boolean(row?.primaryCommand || row?.primaryActionId),
    primaryAction: row?.primaryCommand
      ...
      : (row?.primaryActionId
          ? {
              ...
              label: optionalText(row?.primaryActionId)
            }
          : null),
...
function defaultWorkbenchContextMenuContext({
  ...
  return {
    ...
    rowLabel: optionalText(row?.label || ""),
    record: row?.record ? structuredClone(row.record) : null,
...
function buildWorkbenchContextMenuModel({
  ...
  const workbenchRecord = resolvedContext.record?.scope === "workbench"
    ? resolvedContext.record
    : null;
  const workbenchKind = optionalText(workbenchRecord?.kind);
  const workbenchName = optionalText(
    workbenchRecord?.metadata?.authoredName
    ?? workbenchRecord?.metadata?.name
    ?? null
  );
  const workbenchItems = workbenchKind === "layout"
    ? [
        {
          id: "workbench.layout.save",
          label: "Save",
          shortcut: "1",
          detail: workbenchName ? `layout:${workbenchName}` : "requires active name",
          enabled: Boolean(workbenchName),
          action: { kind: "action-ref", actionId: "workbench.layout.save", context: structuredClone(resolvedContext) }
        },
        {
          id: "workbench.layout.duplicate",
          label: "Duplicate",
          shortcut: "2",
          detail: workbenchName ? `from:${workbenchName}` : "from:current",
          enabled: true,
          action: { kind: "action-ref", actionId: "workbench.layout.duplicate", context: structuredClone(resolvedContext) }
        },
        {
          id: "workbench.layout.rename",
          label: "Rename",
          shortcut: "3",
          detail: workbenchName ? `layout:${workbenchName}` : "needs authored base",
          enabled: Boolean(workbenchName),
          action: { kind: "action-ref", actionId: "workbench.layout.rename", context: structuredClone(resolvedContext) }
        },
        {
          id: "workbench.layout.delete",
          label: "Delete",
          shortcut: "4",
          detail: workbenchName ? `layout:${workbenchName}` : "needs authored base",
          enabled: Boolean(workbenchName),
          action: { kind: "action-ref", actionId: "workbench.layout.delete", context: structuredClone(resolvedContext) }
        },
        {
          id: "workbench.layout.reset",
          label: "Reset",
          shortcut: "5",
          detail: "discard unsaved overlay",
          enabled: true,
          action: { kind: "action-ref", actionId: "workbench.layout.reset", context: structuredClone(resolvedContext) }
        }
      ]
    : (workbenchKind === "keymap"
        ? [
            {
              id: "workbench.keymap.save",
              label: "Save",
              shortcut: "1",
              detail: workbenchName ? `keymap:${workbenchName}` : "requires active name",
              enabled: Boolean(workbenchName),
              action: { kind: "action-ref", actionId: "workbench.keymap.save", context: structuredClone(resolvedContext) }
            },
            {
              id: "workbench.keymap.duplicate",
              label: "Duplicate",
              shortcut: "2",
              detail: workbenchName ? `from:${workbenchName}` : "from:current",
              enabled: true,
              action: { kind: "action-ref", actionId: "workbench.keymap.duplicate", context: structuredClone(resolvedContext) }
            },
            {
              id: "workbench.keymap.rename",
              label: "Rename",
              shortcut: "3",
              detail: workbenchName ? `keymap:${workbenchName}` : "needs authored base",
              enabled: Boolean(workbenchName),
              action: { kind: "action-ref", actionId: "workbench.keymap.rename", context: structuredClone(resolvedContext) }
            },
            {
              id: "workbench.keymap.delete",
              label: "Delete",
              shortcut: "4",
              detail: workbenchName ? `keymap:${workbenchName}` : "needs authored base",
              enabled: Boolean(workbenchName),
              action: { kind: "action-ref", actionId: "workbench.keymap.delete", context: structuredClone(resolvedContext) }
            },
            {
              id: "workbench.keymap.reset",
              label: "Reset",
              shortcut: "5",
              detail: "discard unsaved overlay",
              enabled: true,
              action: { kind: "action-ref", actionId: "workbench.keymap.reset", context: structuredClone(resolvedContext) }
            }
          ]
        : (workbenchKind === "panel"
            ? [
                {
                  ...
                  label: "Open",
                  shortcut: "1",
                  detail: workbenchName ? `panel:${workbenchName}` : "requires panel",
                  enabled: Boolean(workbenchName),
                  action: { kind: "action-ref", actionId: "workbench.panel.open", context: structuredClone(resolvedContext) }
                },
                {
                  id: "workbench.panel.duplicate",
                  label: "Duplicate",
                  shortcut: "2",
                  detail: workbenchName ? `from:${workbenchName}` : "requires panel",
                  enabled: Boolean(workbenchName),
                  action: { kind: "action-ref", actionId: "workbench.panel.duplicate", context: structuredClone(resolvedContext) }
                },
                {
                  id: "workbench.panel.rename",
                  label: "Rename",
                  shortcut: "3",
                  detail: workbenchName ? `panel:${workbenchName}` : "requires panel",
                  enabled: Boolean(workbenchName),
                  action: { kind: "action-ref", actionId: "workbench.panel.rename", context: structuredClone(resolvedContext) }
                },
                {
                  id: "workbench.panel.delete",
                  label: "Delete",
                  shortcut: "4",
                  detail: workbenchName ? `panel:${workbenchName}` : "requires panel",
                  enabled: Boolean(workbenchName),
                  action: { kind: "action-ref", actionId: "workbench.panel.delete", context: structuredClone(resolvedContext) }
                },
                {
                  id: "workbench.panel.assign-content",
                  label: "Assign Content",
                  shortcut: "5",
                  detail: "pick alternate content",
                  enabled: Boolean(workbenchName),
                  action: { kind: "action-ref", actionId: "workbench.panel.assign-content", context: structuredClone(resolvedContext) }
                }
              ]
            : (workbenchKind === "content"
                ? [
                    {
                      id: "workbench.content.open",
                      label: "Open",
                      shortcut: "1",
                      detail: workbenchName ? `content:${workbenchName}` : "requires content",
                      enabled: Boolean(workbenchName),
                      action: { kind: "action-ref", actionId: "workbench.content.open", context: structuredClone(resolvedContext) }
                    },
                    {
                      id: "workbench.content.duplicate",
                      label: "Duplicate",
                      shortcut: "2",
                      detail: workbenchName ? `from:${workbenchName}` : "requires content",
                      enabled: Boolean(workbenchName),
                      action: { kind: "action-ref", actionId: "workbench.content.duplicate", context: structuredClone(resolvedContext) }
                    },
                    {
                      id: "workbench.content.rename",
                      label: "Rename",
                      shortcut: "3",
                      detail: workbenchName ? `content:${workbenchName}` : "requires content",
                      enabled: Boolean(workbenchName),
                      action: { kind: "action-ref", actionId: "workbench.content.rename", context: structuredClone(resolvedContext) }
                    },
                    {
                      id: "workbench.content.delete",
                      label: "Delete",
                      shortcut: "4",
                      detail: workbenchName ? `content:${workbenchName}` : "requires content",
                      enabled: Boolean(workbenchName),
                      action: { kind: "action-ref", actionId: "workbench.content.delete", context: structuredClone(resolvedContext) }
                    }
                  ]
                : null)));
  const authoredMenu = surfaceSpec?.menuId
    ? workbenchDefinition?.menusById?.get(surfaceSpec.menuId) ?? null
    : null;
  const items = Array.isArray(workbenchItems)
    ? workbenchItems
    : arrayWrap(authoredMenu?.itemActionIds).length
    ? authoredMenu.itemActionIds
      .map((actionId, index) => {
        const actionSpec = workbenchDefinition?.actionsById?.get(actionId) ?? null;
        if (!actionSpec) return null;
        return {
          id: actionSpec.id,
          label: actionSpec.title ?? actionSpec.id,
          shortcut: String(index + 1),
          detail: actionSpec.kind === "builtin"
            ? (actionSpec.builtin === "set-right-screen"
                ? `screen:${actionSpec.screenId ?? "(none)"}`
                : actionSpec.builtin)
            : `${actionSpec.kind}${arrayWrap(actionSpec.steps).length ? `:${actionSpec.steps.length}` : ""}`,
          enabled: actionSpec.builtin === "change-color"
            ? pane !== "bottom"
            : (["rename", "edit", "clone"].includes(actionSpec.builtin) ? actionable : true),
          action: {
            kind: "action-ref",
  ...
  return {
    ...overlay,
    title: authoredMenu?.title ?? overlay.title,
    subject,
...
export async function buildOperatorWorkbenchSnapshot(state, session, uiState = {}) {
  uiState = {
    ...(session.uiState && typeof session.uiState === "object" ? structuredClone(session.uiState) : {}),
  ...
  const activeLeftDatasetSpec = activeLeftScreenSpec?.datasetId
    ...
    : null;
  const activeLayoutRecord = currentWorkbenchLayoutRecord(state, session);
  const activeKeymapRecord = currentWorkbenchKeymapRecord(session);
  const activeViewportId = activeLayoutRecord.viewportId ?? workbenchDefinition.defaultViewport ?? workbenchDefinition.viewports?.[0]?.id ?? null;
  ...
  const focusedPanelId = optionalText(session.workbenchFocusedPanelId ?? activeLayoutRecord.focusedPanelId) ?? null;
  const topPaneSnapshot = workbenchTopPaneModel({
    topNavigationModel,
  ...
  const bottomPaneSnapshot = workbenchBottomPaneModel(activeScreenSpec?.id ?? activeScreenId);
  const leftPaneSnapshot = {
  ...
  const panelSnapshots = buildWorkbenchPanelSnapshots(state, session, {
    layoutRecord: activeLayoutRecord,
    leftPaneSnapshot,
    rightPaneSnapshot,
    activeKeymapRecord
  });
  const chromeSnapshots = buildWorkbenchChromeSnapshots(activeViewportSpec, {
    topPaneSnapshot,
    bottomPaneSnapshot,
    focusedPane
  });
  const rootSplitSnapshot = materializeWorkbenchSplitNode(activeLayoutRecord.root);
  const windowSnapshot = activeViewportSpec
    ? {
        ...
        top: activeViewportLayout?.top ?? activeViewportSpec.top ?? null,
        bottom: activeViewportLayout?.bottom ?? activeViewportSpec.bottom ?? null,
        topChromeId: activeViewportSpec.topSurfaceId ?? null,
        bottomChromeId: activeViewportSpec.bottomSurfaceId ?? null,
  ...
  return {
    ...
    ui: {
      ...
      focusedPanelId: optionalText(session.workbenchFocusedPanelId ?? activeLayoutRecord.focusedPanelId) ?? null,
    ...
    rootSplit: rootSplitSnapshot,
    panels: panelSnapshots,
    chromes: chromeSnapshots,
    focusedPanelId,
    activeLayout: structuredClone(activeLayoutRecord),
    activeKeymap: structuredClone(activeKeymapRecord),
    ...
    actions: {
      available: [
        ...workbenchDefinition.actions.map(action => ({
          id: action.id,
          title: action.title ?? action.id,
          kind: action.kind,
          builtin: action.builtin ?? null,
          ...
          message: action.message ?? null,
          steps: structuredClone(action.steps ?? []),
          origin: action.origin ?? "authored"
        })),
        ...SYNTHETIC_WORKBENCH_ACTION_SPECS.map(action => ({
          ...action,
          overlayId: null,
          screenId: null,
          pane: null,
      ...
      menus: workbenchDefinition.menus.map(menu => ({
        id: menu.id,
        title: menu.title ?? menu.id,
        itemActionIds: structuredClone(menu.itemActionIds ?? []),
        origin: menu.origin ?? "authored"
      }))
    },
    workbench: {
      groups: buildWorkbenchGroups(state, session).map(group => ({
        id: group.id,
        label: group.label,
        count: group.count,
        summary: group.summary ?? ""
      })),
      sessionGroups: buildSessionGroups(session).map(group => ({
        id: group.id,
        label: group.label,
        count: group.count
      })),
      layouts: workbenchLayoutRecords(state, session).map(record => structuredClone(record)),
      keymaps: workbenchKeymapRecords(state, session).map(record => structuredClone(record)),
      viewports: workbenchViewportRecords(state).map(record => structuredClone(record)),
      panels: workbenchPanelRecords(state, session).map(record => structuredClone(record)),
      contents: workbenchContentRecords(state).map(record => structuredClone(record)),
      splits: workbenchSplitRecords(state).map(record => structuredClone(record)),
      windows: workbenchWindowRecords(state).map(record => structuredClone(record)),
      chromes: workbenchChromeRecords(state).map(record => structuredClone(record)),
      activeLayout: structuredClone(activeLayoutRecord),
      activeKeymap: structuredClone(activeKeymapRecord),
      focusedPanelId
    },
    viewport: activeViewportSpec
      ? {
          ...
          root: rootSplitSnapshot,
          panels: structuredClone(activeLayoutRecord.panels),
          focusedPanelId,
          overlays: structuredClone(activeViewportSpec.overlays ?? []),
          bindings: structuredClone(activeViewportSpec.bindings ?? []),
          keybindings: structuredClone(activeKeymapRecord.bindings ?? {})
        }
      : null,
    topPane: topPaneSnapshot,
    bottomPane: bottomPaneSnapshot,
    helpOverlay: helpOverlayModel,
    overlays: overlayModels,
    leftPane: leftPaneSnapshot,
...
function autocompleteWorkbenchLayoutNames(session) {
  return stableSortStrings(Object.keys(session.savedWorkbenchLayouts ?? {}));
}

function autocompleteWorkbenchKeymapNames(session) {
  return stableSortStrings(Object.keys(session.savedWorkbenchKeymaps ?? {}));
}
...
export function buildTuiAutocompleteCandidates(state, session) {
  ...
  const layoutNames = autocompleteWorkbenchLayoutNames(session);
  const keymapNames = autocompleteWorkbenchKeymapNames(session);
  const panelNames = stableSortStrings(arrayWrap(state.workbenchAuthoringSpecs?.panels).map(panel => panel?.id).filter(Boolean));
  const contentNames = stableSortStrings(arrayWrap(state.workbenchAuthoringSpecs?.contents).map(content => content?.id).filter(Boolean));
  ...
  const actionIds = operatorWorkbenchDefinitionForState(state).actions.map(action => action.id);
  return uniqueStrings([
    "status",
    "tree",
    "look",
    ...
    "split vertical",
    "panel open ",
    ...panelNames.map(name => `panel open ${name}`),
    "panel duplicate ",
    ...panelNames.map(name => `panel duplicate ${name} `),
    "panel rename ",
    ...panelNames.map(name => `panel rename ${name} `),
    ...panelNames.map(name => `panel delete ${name}`),
    ...panelNames.flatMap(name => contentNames.map(contentName => `panel assign-content ${name} ${contentName}`)),
    "panel delete ",
    "panel assign-content ",
    "panels",
    "panel close",
    "panel focus next",
    "panel focus prev",
    "panel focus ",
    "content open ",
    ...contentNames.map(name => `content open ${name}`),
    "content duplicate ",
    ...contentNames.map(name => `content duplicate ${name} `),
    "content rename ",
    ...contentNames.map(name => `content rename ${name} `),
    ...contentNames.map(name => `content delete ${name}`),
    "content delete ",
    "contents",
    "layout new ",
    "layout duplicate ",
    "layout rename ",
    "layout save ",
    ...layoutNames.map(name => `layout open ${name}`),
    ...layoutNames.map(name => `layout duplicate ${name} `),
    ...layoutNames.map(name => `layout rename ${name} `),
    ...layoutNames.map(name => `layout delete ${name}`),
    "layout open ",
    "layout delete ",
    "layouts",
    "layout reset",
    ...
    "keymap rename ",
    "keymap save ",
    ...keymapNames.map(name => `keymap open ${name}`),
    ...keymapNames.map(name => `keymap duplicate ${name} `),
    ...keymapNames.map(name => `keymap rename ${name} `),
    ...keymapNames.map(name => `keymap delete ${name}`),
    "keymap open ",
    "keymap delete ",
    "keymaps",
    "keymap reset",
    "bind ",
    ...actionIds.map(actionId => `bind F1 ${actionId}`),
    "unbind ",
    "refresh",
    "leave",
    "help",
...
function materializeWorkbenchSplitNode(root = null, path = "root") {
  if (!root || typeof root !== "object") return null;
  if (root.kind === "panel") {
    const panelId = optionalText(root.panelId);
    return panelId ? { kind: "panel", panelId } : null;
  ...
  if (root.kind !== "split") return null;
  const first = materializeWorkbenchSplitNode(root.first, `${path}.first`);
  const second = materializeWorkbenchSplitNode(root.second, `${path}.second`);
  if (!first || !second) return null;
  return {
    kind: "split",
    id: optionalText(root.id) ?? `split:${path}`,
    axis: optionalText(root.axis) === "horizontal" ? "horizontal" : "vertical",
    weight: Math.min(85, Math.max(15, Math.round(Number(root.weight ?? 50) || 50))),
    handle: optionalText(root.handle) ?? null,
    first,
...
function workbenchPanelRoleForSnapshot(state, panelState = null, panelId = null) {
  const definition = operatorWorkbenchDefinitionForState(state);
  const authored = panelId ? (definition.panelsById?.get(panelId) ?? null) : null;
  const authoredRole = optionalText(authored?.role);
  if (authoredRole === "left" || authoredRole === "right" || authoredRole === "aux") return authoredRole;
  if (optionalText(panelState?.contentKind) === "left-screen") return "left";
  return "right";
}

function workbenchPanelPrimaryActionSnapshot(state, keymapRecord, panelState = null, panelId = null) {
  const actionId = optionalText(
    keymapRecord?.panelPrimaryActions?.[panelId]
    ?? panelState?.primaryActionId
  );
  if (!actionId) return null;
  const action = operatorWorkbenchActionSpec(state, actionId) ?? null;
  return {
    id: actionId,
...
function buildWorkbenchPanelSnapshots(state, session, {
  layoutRecord,
  leftPaneSnapshot,
  rightPaneSnapshot,
  activeKeymapRecord
} = {}) {
  const panelIds = collectLayoutPanelIds(layoutRecord?.root);
  return panelIds.map(panelId => {
    const panelState = layoutRecord?.panels?.[panelId] ?? null;
    const role = workbenchPanelRoleForSnapshot(state, panelState, panelId);
    const contentKind = optionalText(panelState?.contentKind) ?? (role === "left" ? "left-screen" : "screen");
    const contentModel = contentKind === "left-screen"
      ? structuredClone(leftPaneSnapshot ?? {})
      : structuredClone(rightPaneSnapshot ?? {});
    const activeRow = contentKind === "left-screen"
      ? contentModel?.activeRow ?? null
      : contentModel?.screen?.rows?.[contentModel?.cursor ?? 0] ?? null;
    const actionable = contentKind === "left-screen"
      ? Boolean(contentModel?.activeRow?.primaryAction || contentModel?.activeRow?.actionable)
      : Boolean(activeRow?.primaryCommand || activeRow?.primaryActionId || contentModel?.screen?.activeSectionActionable);
    return {
      id: panelId,
      role,
      title: optionalText(panelState?.title)
        ?? optionalText(contentModel?.title)
        ?? panelId,
      contentKind,
      contentModel,
      primaryAction: workbenchPanelPrimaryActionSnapshot(state, activeKeymapRecord, panelState, panelId),
      secondaryMenuId: optionalText(
        activeKeymapRecord?.panelSecondaryMenus?.[panelId]
        ?? panelState?.secondaryMenuId
      ) ?? null,
      cursor: contentKind === "left-screen"
        ? Number(contentModel?.cursor ?? contentModel?.activeRowIndex ?? 0) || 0
        : Number(contentModel?.cursor ?? contentModel?.screen?.activeRowIndex ?? 0) || 0,
      actionable,
      focused: optionalText(session.workbenchFocusedPanelId ?? layoutRecord?.focusedPanelId) === panelId
    };
  });
}

function buildWorkbenchChromeSnapshots(activeViewportSpec, {
  topPaneSnapshot,
  bottomPaneSnapshot,
  focusedPane
} = {}) {
  const chromes = [];
  if (activeViewportSpec?.topSurfaceId) {
    chromes.push({
      id: activeViewportSpec.topSurfaceId,
      role: "top",
      title: optionalText(topPaneSnapshot?.title) ?? "Status",
      contentKind: "chrome",
      chromeKind: "top",
      contentModel: structuredClone(topPaneSnapshot ?? {}),
      focused: focusedPane === "top",
      actionable: true
    });
  }
  if (activeViewportSpec?.bottomSurfaceId) {
    chromes.push({
      id: activeViewportSpec.bottomSurfaceId,
      role: "bottom",
      title: optionalText(bottomPaneSnapshot?.title) ?? "Commands",
      contentKind: "chrome",
      chromeKind: "bottom",
      contentModel: structuredClone(bottomPaneSnapshot ?? {}),
      focused: focusedPane === "bottom",
      actionable: true
    });
  }
  return chromes;
...
function collectLayoutPanelIds(root, rows = []) {
  if (!root || typeof root !== "object") return rows;
  if (root.kind === "panel" && optionalText(root.panelId)) {
    rows.push(root.panelId);
    return rows;
  }
  if (root.kind === "split") {
    collectLayoutPanelIds(root.first, rows);
    collectLayoutPanelIds(root.second, rows);
  }
  return rows;
}
...
function createEmptySession() {
  return {
    ...
    savedWorkbenchKeymaps: {},
    activeWorkbenchLayoutName: null,
    activeWorkbenchKeymapName: null,
    workbenchLayoutOverlay: null,
    workbenchBindingOverrides: {},
    workbenchPanelPrimaryActionOverrides: {},
    workbenchPanelSecondaryMenuOverrides: {},
    workbenchFocusedPanelId: null,
    uiState: {}
  };
}
...
function buildWorkbenchState(runtimeContext) {
  ...
  const platformModelProject = typeof workbenchWorld?.project === "function"
    ...
    : (() => []);
  const workbenchAuthoringSpecs = runtimeContext?.appProject
    ? collectAuthoredOperatorWorkbenchSpecs(runtimeContext.appProject.authoredDesireDocs ?? [], {
  ...
  return buildPlatformModel({
    ...
    return {
      ...
      workbenchAuthoringSpecs,
      workbenchDefinitionOverride: null
    };
  });
}
...
export function createOperatorTuiEngine(state, session = createEmptySession()) {
  const engine = {
    ...
    session,
    async rebuild() {
      const preservedWorkbenchSpecs = state.workbenchAuthoringSpecs ? deepClone(state.workbenchAuthoringSpecs) : null;
      ...
      state.workbenchAuthoringSpecs = nextState.workbenchAuthoringSpecs;
      state.workbenchDefinitionOverride = nextState.workbenchDefinitionOverride;
      if (preservedWorkbenchSpecs) {
        setOperatorWorkbenchAuthoringSpecs(state, preservedWorkbenchSpecs);
      }
  ...
  function requeryResultView(resultView, { activeViewName } = {}) {
    const nextRows = searchRecords(state, {
      ...
      scope: resultView.scope,
      focusContextId: resultView.focusContextId ?? null,
      includeWorkbench: includeWorkbenchRecordsForSearch(session),
      session
    });
  ...
  function activeWorkbenchLayout() {
    return currentWorkbenchLayoutRecord(state, session);
  }

  function ensureWorkbenchLayoutOverlay() {
    if (!session.workbenchLayoutOverlay) {
      session.workbenchLayoutOverlay = cloneWorkbenchLayoutRecord(activeWorkbenchLayout(), "__overlay__");
    }
    return session.workbenchLayoutOverlay;
  }

  function activeWorkbenchKeymap() {
    return currentWorkbenchKeymapRecord(session);
  }

  function validWorkbenchObjectName(value) {
    return /^[A-Za-z][A-Za-z0-9_-]*$/.test(String(value ?? "").trim());
  }

  function nextWorkbenchCopyName(existingNames, baseName) {
    const normalizedBase = optionalText(baseName) ?? "copy";
    let nextName = `${normalizedBase}-copy`;
    let index = 2;
    while (existingNames.has(nextName)) {
      nextName = `${normalizedBase}-copy-${index}`;
      index += 1;
    }
    return nextName;
  }

  function clearWorkbenchLayoutOverlay({ preserveActiveName = true } = {}) {
    if (!preserveActiveName) session.activeWorkbenchLayoutName = null;
    session.workbenchLayoutOverlay = null;
    session.workbenchFocusedPanelId = null;
  }

  function clearWorkbenchKeymapOverlay({ preserveActiveName = true } = {}) {
    if (!preserveActiveName) session.activeWorkbenchKeymapName = null;
    session.workbenchBindingOverrides = {};
    session.workbenchPanelPrimaryActionOverrides = {};
    session.workbenchPanelSecondaryMenuOverrides = {};
  }

  function resolveLayoutCatalogRecord(token) {
    const normalized = optionalText(token);
    if (!normalized) return null;
    if (normalized === "current") {
      const current = cloneWorkbenchLayoutRecord(activeWorkbenchLayout(), session.activeWorkbenchLayoutName ?? "current");
      return {
        name: session.activeWorkbenchLayoutName ?? null,
        current: true,
        record: current
    ...
    const record = cloneWorkbenchLayoutRecord(session.savedWorkbenchLayouts?.[normalized] ?? null, normalized);
    if (!record) return null;
    return {
      name: normalized,
      current: false,
  ...
  function resolveKeymapCatalogRecord(token) {
    const normalized = optionalText(token);
    if (!normalized) return null;
    if (normalized === "current") {
      const current = cloneWorkbenchKeymapRecord(activeWorkbenchKeymap(), session.activeWorkbenchKeymapName ?? "current");
      return {
        name: session.activeWorkbenchKeymapName ?? null,
        current: true,
        record: current
    ...
    const record = cloneWorkbenchKeymapRecord(session.savedWorkbenchKeymaps?.[normalized] ?? null, normalized);
    if (!record) return null;
    return {
      name: normalized,
      current: false,
  ...
  function cloneAuthoredPanelRecord(record = null, fallbackName = null) {
    if (!record || typeof record !== "object") return null;
    return {
      ...deepClone(record),
      id: optionalText(record.id ?? fallbackName) ?? fallbackName ?? null
    };
  }

  function cloneAuthoredContentRecord(record = null, fallbackName = null) {
    if (!record || typeof record !== "object") return null;
    return {
      ...deepClone(record),
      id: optionalText(record.id ?? fallbackName) ?? fallbackName ?? null
  ...
  function authoredPanelCatalog() {
    return Object.fromEntries(
      arrayWrap(state.workbenchAuthoringSpecs?.panels)
        .map(panel => [panel?.id, cloneAuthoredPanelRecord(panel, panel?.id)])
        .filter(([name, panel]) => Boolean(optionalText(name)) && Boolean(panel))
    );
  }

  function authoredContentCatalog() {
    return Object.fromEntries(
      arrayWrap(state.workbenchAuthoringSpecs?.contents)
        .map(content => [content?.id, cloneAuthoredContentRecord(content, content?.id)])
        .filter(([name, content]) => Boolean(optionalText(name)) && Boolean(content))
    );
  }

  function resolvePanelCatalogRecord(token) {
    const normalized = optionalText(token);
    if (!normalized) return null;
    const record = cloneAuthoredPanelRecord(authoredPanelCatalog()[normalized] ?? null, normalized);
    if (!record) return null;
    return {
      name: normalized,
      current: false,
  ...
  function resolveContentCatalogRecord(token) {
    const normalized = optionalText(token);
    if (!normalized) return null;
    const record = cloneAuthoredContentRecord(authoredContentCatalog()[normalized] ?? null, normalized);
    if (!record) return null;
    return {
      name: normalized,
      current: false,
  ...
  function workbenchPanelRecordByName(name) {
    const normalized = optionalText(name);
    if (!normalized) return null;
    return workbenchPanelRecords(state, session)
      .find(record => optionalText(record?.metadata?.authoredName) === normalized)
      ?? null;
  }

  function workbenchContentRecordByName(name) {
    const normalized = optionalText(name);
    if (!normalized) return null;
    return workbenchContentRecords(state)
      .find(record => optionalText(record?.metadata?.authoredName) === normalized)
      ?? null;
  }

  function currentWorkbenchSpecs() {
    return deepClone(state.workbenchAuthoringSpecs ?? {
      themes: [],
      datasets: [],
      actions: [],
      menus: [],
  ...
  function panelReferrersInSpecs(specs, panelId) {
    const refs = [];
    for (const split of arrayWrap(specs?.splits)) {
      if (split?.first === panelId || split?.second === panelId) refs.push(`split:${split.id}`);
    }
    for (const windowSpec of arrayWrap(specs?.windows)) {
      if (windowSpec?.leftPanelId === panelId || windowSpec?.rightPanelId === panelId) refs.push(`window:${windowSpec.id}`);
    }
    for (const content of arrayWrap(specs?.contents)) {
      if (content?.leftPanelId === panelId) refs.push(`content:${content.id}`);
    }
    return refs;
  }

  function contentReferrersInSpecs(specs, contentId) {
    const refs = [];
    for (const panel of arrayWrap(specs?.panels)) {
      if (panel?.contentId === contentId) refs.push(`panel:${panel.id}`);
    }
    for (const chrome of arrayWrap(specs?.chromes)) {
      if (chrome?.contentId === contentId) refs.push(`chrome:${chrome.id}`);
    }
    for (const content of arrayWrap(specs?.contents)) {
      if (arrayWrap(content?.sectionIds).includes(contentId)) refs.push(`content:${content.id}:section`);
      if (content?.defaultSectionId === contentId) refs.push(`content:${content.id}:default-section`);
    }
    return refs;
  }

  function rewritePanelReferencesInSpecs(specs, sourceId, targetId) {
    for (const split of arrayWrap(specs?.splits)) {
  ...
  function rewriteContentReferencesInSpecs(specs, sourceId, targetId) {
    for (const panel of arrayWrap(specs?.panels)) {
    ...
    for (const content of arrayWrap(specs?.contents)) {
      if (arrayWrap(content?.sectionIds).includes(sourceId)) {
        content.sectionIds = arrayWrap(content.sectionIds).map(sectionId => sectionId === sourceId ? targetId : sectionId);
      }
      if (content?.defaultSectionId === sourceId) content.defaultSectionId = targetId;
    }
  }

  async function inspectWorkbenchRecordByName(kind, name) {
    const record = kind === "panel"
      ? workbenchPanelRecordByName(name)
      : workbenchContentRecordByName(name);
    if (!record) {
      return {
        output: `${kind} not found: ${name}`,
        exit: false,
        status: "error"
      };
    }
    const inspected = await inspectRecordForWorkbench(state, session, record);
    return {
      output: inspected.inspectLines.join("\n"),
      exit: false,
      ui: {
        ...builtInScreenUi("inspect"),
        inspectorSpec: {
          kind: inspected.target?.kind ?? deepLinkKindForRecord(record),
          targetId: record.id
        }
      }
    };
  ...
  function replaceLayoutPanel(root, panelId, nextNode) {
    if (!root || typeof root !== "object") return null;
    if (root.kind === "panel") {
      return root.panelId === panelId ? deepClone(nextNode) : deepClone(root);
    }
    return {
      kind: "split",
      axis: root.axis === "horizontal" ? "horizontal" : "vertical",
      weight: Math.min(85, Math.max(15, Math.round(Number(root.weight ?? 50) || 50))),
      first: replaceLayoutPanel(root.first, panelId, nextNode),
      second: replaceLayoutPanel(root.second, panelId, nextNode)
    };
  }

  function removeLayoutPanel(root, panelId) {
    if (!root || typeof root !== "object") return null;
    if (root.kind === "panel") {
      return root.panelId === panelId ? null : deepClone(root);
    }
    const first = removeLayoutPanel(root.first, panelId);
    const second = removeLayoutPanel(root.second, panelId);
    if (!first && !second) return null;
  ...
  function nextGeneratedPanelId(layout) {
    const existing = new Set(Object.keys(layout?.panels ?? {}));
    let index = existing.size + 1;
    while (existing.has(`panel.${index}`)) index += 1;
    return `panel.${index}`;
  }

  function duplicatePanelState(panel, panelId) {
    const next = cloneWorkbenchPanelState(panel, panelId);
    if (!next) return null;
    next.id = panelId;
    next.title = panel?.title ? `${panel.title} Copy` : panelId;
    return next;
  }
  ...
  async function executeInternal(command, options = {}) {
    if (command === "help") {
      return {
        output: [
          ...
          "  split vertical",
          "  panel open <name>",
          "  panel duplicate <source> <name>",
          "  panel rename <source> <name>",
          "  panel delete <name>",
          "  panel assign-content <panel> <content>",
          "  panels",
          "  panel close",
          "  panel focus next|prev|<id>",
          "  content open <name>",
          "  content duplicate <source> <name>",
          "  content rename <source> <name>",
          "  content delete <name>",
          "  contents",
          "  layout new <name>",
          "  layout duplicate <source> <name>",
          "  layout rename <source> <name>",
          "  layout save <name>",
          "  layout open <name>",
          "  layout delete <name>",
          "  layouts",
          "  layout reset",
          "  keymap new <name>",
          "  keymap duplicate <source> <name>",
          "  keymap rename <source> <name>",
          "  keymap save <name>",
          "  keymap open <name>",
          "  keymap delete <name>",
          "  keymaps",
          "  keymap reset",
          "  bind <trigger> <action-id>",
          "  unbind <trigger>",
          "  refresh",
          "  quit"
        ].join("\n"),
    ...
    const splitMatch = command.match(/^split\s+(horizontal|vertical)$/);
    if (splitMatch) {
      const layout = ensureWorkbenchLayoutOverlay();
      const focusedPanelId = optionalText(session.workbenchFocusedPanelId ?? layout.focusedPanelId) ?? collectLayoutPanelIds(layout.root)[0] ?? null;
      if (!focusedPanelId || !layout.panels?.[focusedPanelId]) {
        return { output: "no active panel.", exit: false, status: "error" };
      }
      const nextPanelId = nextGeneratedPanelId(layout);
      const duplicate = duplicatePanelState(layout.panels[focusedPanelId], nextPanelId);
      if (!duplicate) {
        return { output: "unable to duplicate active panel.", exit: false, status: "error" };
      }
      layout.panels[nextPanelId] = duplicate;
      layout.root = replaceLayoutPanel(layout.root, focusedPanelId, {
        kind: "split",
        axis: splitMatch[1] === "horizontal" ? "horizontal" : "vertical",
        weight: 50,
        first: { kind: "panel", panelId: focusedPanelId },
        second: { kind: "panel", panelId: nextPanelId }
      });
      session.workbenchFocusedPanelId = nextPanelId;
      return {
        output: `split ${focusedPanelId} ${splitMatch[1]}; new panel ${nextPanelId}.`,
        exit: false
      };
    }

    if (command === "panel close") {
      const layout = ensureWorkbenchLayoutOverlay();
      const focusedPanelId = optionalText(session.workbenchFocusedPanelId ?? layout.focusedPanelId) ?? collectLayoutPanelIds(layout.root)[0] ?? null;
      const panelIds = collectLayoutPanelIds(layout.root);
      if (!focusedPanelId || !panelIds.includes(focusedPanelId)) {
        return { output: "no active panel.", exit: false, status: "error" };
      }
      if (panelIds.length <= 1) {
        return { output: "cannot close the last panel.", exit: false, status: "error" };
      }
      layout.root = removeLayoutPanel(layout.root, focusedPanelId);
      delete layout.panels[focusedPanelId];
      const remaining = collectLayoutPanelIds(layout.root);
      session.workbenchFocusedPanelId = remaining[0] ?? null;
      return {
        output: `closed panel ${focusedPanelId}.`,
        exit: false
      };
    }

    const panelFocusMatch = command.match(/^panel\s+focus\s+(.+)$/);
    if (panelFocusMatch) {
      const layout = activeWorkbenchLayout();
      const panelIds = collectLayoutPanelIds(layout.root);
      if (!panelIds.length) return { output: "no panels in the active layout.", exit: false, status: "error" };
      const token = optionalText(panelFocusMatch[1]);
      const currentPanelId = optionalText(session.workbenchFocusedPanelId ?? layout.focusedPanelId) ?? panelIds[0];
      let nextPanelId = null;
      if (token === "next" || token === "prev") {
        const currentIndex = Math.max(0, panelIds.indexOf(currentPanelId));
        const delta = token === "prev" ? -1 : 1;
        nextPanelId = panelIds[(currentIndex + delta + panelIds.length) % panelIds.length] ?? null;
      } else {
        nextPanelId = panelIds.find(id => id === token) ?? null;
      }
      if (!nextPanelId) {
        return { output: `panel not found: ${token}`, exit: false, status: "error" };
      }
      session.workbenchFocusedPanelId = nextPanelId;
      return { output: `focused panel ${nextPanelId}.`, exit: false };
    }

    const panelOpenMatch = command.match(/^panel\s+open\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (panelOpenMatch) {
      return inspectWorkbenchRecordByName("panel", panelOpenMatch[1]);
    }

    const panelDuplicateMatch = command.match(/^panel\s+duplicate\s+([A-Za-z][A-Za-z0-9_-]*)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (panelDuplicateMatch) {
      const sourceName = panelDuplicateMatch[1];
      const targetName = panelDuplicateMatch[2];
      const panels = authoredPanelCatalog();
      if (panels[targetName]) {
        return { output: `panel already exists: ${targetName}`, exit: false, status: "error" };
      }
      const source = cloneAuthoredPanelRecord(panels[sourceName] ?? null, sourceName);
      if (!source) {
        return { output: `panel not found: ${sourceName}`, exit: false, status: "error" };
      }
      const nextSpecs = currentWorkbenchSpecs();
      nextSpecs.panels = [
        ...arrayWrap(nextSpecs.panels).filter(panel => panel?.id !== targetName),
        {
          ...source,
          id: targetName,
      ...
      setOperatorWorkbenchAuthoringSpecs(state, nextSpecs);
      return {
        output: `duplicated panel ${sourceName} as ${targetName}.`,
        exit: false,
        workbenchMutation: {
          kind: "save-panel",
          name: targetName,
          record: cloneAuthoredPanelRecord(nextSpecs.panels.find(panel => panel?.id === targetName) ?? null, targetName)
        }
      };
    }

    const panelRenameMatch = command.match(/^panel\s+rename\s+([A-Za-z][A-Za-z0-9_-]*)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (panelRenameMatch) {
      const sourceName = panelRenameMatch[1];
      const targetName = panelRenameMatch[2];
      const panels = authoredPanelCatalog();
      if (!panels[sourceName]) {
        return { output: `panel not found: ${sourceName}`, exit: false, status: "error" };
      }
      if (sourceName === targetName) {
        return { output: `panel already named ${targetName}.`, exit: false, status: "error" };
      }
      if (panels[targetName]) {
        return { output: `panel already exists: ${targetName}`, exit: false, status: "error" };
      }
      const nextSpecs = currentWorkbenchSpecs();
      nextSpecs.panels = arrayWrap(nextSpecs.panels)
        .filter(panel => panel?.id !== sourceName)
        .concat([{
          ...cloneAuthoredPanelRecord(panels[sourceName], targetName),
          id: targetName,
          title: panels[sourceName]?.title === sourceName ? targetName : panels[sourceName]?.title
        }]);
      rewritePanelReferencesInSpecs(nextSpecs, sourceName, targetName);
      setOperatorWorkbenchAuthoringSpecs(state, nextSpecs);
      return {
        output: `renamed panel ${sourceName} to ${targetName}.`,
        exit: false,
        workbenchMutation: {
          kind: "rename-panel",
          sourceName,
    ...
    const panelDeleteMatch = command.match(/^panel\s+delete\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (panelDeleteMatch) {
      const name = panelDeleteMatch[1];
      const panels = authoredPanelCatalog();
      if (!panels[name]) {
        return { output: `panel not found: ${name}`, exit: false, status: "error" };
      }
      const refs = panelReferrersInSpecs(state.workbenchAuthoringSpecs, name);
      if (refs.length) {
        return {
          output: `panel ${name} is still referenced by ${refs.join(", ")}`,
          exit: false,
          status: "error"
        };
      }
      const nextSpecs = currentWorkbenchSpecs();
      nextSpecs.panels = arrayWrap(nextSpecs.panels).filter(panel => panel?.id !== name);
      setOperatorWorkbenchAuthoringSpecs(state, nextSpecs);
      return {
        output: `deleted panel ${name}.`,
        exit: false,
        workbenchMutation: {
          kind: "delete-panel",
          name
    ...
    const panelAssignContentMatch = command.match(/^panel\s+assign-content\s+([A-Za-z][A-Za-z0-9_-]*)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (panelAssignContentMatch) {
      const panelName = panelAssignContentMatch[1];
      const contentName = panelAssignContentMatch[2];
      const panels = authoredPanelCatalog();
      const contents = authoredContentCatalog();
      if (!panels[panelName]) {
        return { output: `panel not found: ${panelName}`, exit: false, status: "error" };
      }
      if (!contents[contentName]) {
        return { output: `content not found: ${contentName}`, exit: false, status: "error" };
      }
      const nextSpecs = currentWorkbenchSpecs();
      nextSpecs.panels = arrayWrap(nextSpecs.panels).map(panel => panel?.id === panelName
        ? {
            ...panel,
      ...
      setOperatorWorkbenchAuthoringSpecs(state, nextSpecs);
      return {
        output: `assigned content ${contentName} to panel ${panelName}.`,
        exit: false,
        workbenchMutation: {
          kind: "assign-panel-content",
          name: panelName,
          contentName,
          record: cloneAuthoredPanelRecord(nextSpecs.panels.find(panel => panel?.id === panelName) ?? null, panelName)
        }
      };
    }

    if (command === "panels") {
      const rows = workbenchPanelRecords(state, session);
      return {
        output: rows.length
          ? rows.map(record => `${record.title} :: role=${record.metadata?.role ?? "aux"} | content=${record.metadata?.contentId ?? "(none)"}`).join("\n")
          : "(no panels)",
        exit: false
      };
    }

    const layoutNewMatch = command.match(/^layout\s+new\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (layoutNewMatch) {
      const name = layoutNewMatch[1];
      if (session.savedWorkbenchLayouts?.[name]) {
        return { output: `layout already exists: ${name}`, exit: false, status: "error" };
      }
      const layout = cloneWorkbenchLayoutRecord(activeWorkbenchLayout(), name);
      layout.name = name;
      layout.savedAt = timestamp();
      session.savedWorkbenchLayouts[name] = layout;
      session.activeWorkbenchLayoutName = name;
      clearWorkbenchLayoutOverlay({ preserveActiveName: true });
      session.workbenchFocusedPanelId = optionalText(layout.focusedPanelId) ?? null;
      return {
        output: `created layout ${name}.`,
        exit: false,
        workbenchMutation: {
          kind: "save-layout",
          name,
    ...
    const layoutDuplicateMatch = command.match(/^layout\s+duplicate\s+([A-Za-z][A-Za-z0-9_-]*|current)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (layoutDuplicateMatch) {
      const sourceToken = layoutDuplicateMatch[1];
      const name = layoutDuplicateMatch[2];
      if (session.savedWorkbenchLayouts?.[name]) {
        return { output: `layout already exists: ${name}`, exit: false, status: "error" };
      }
      const source = resolveLayoutCatalogRecord(sourceToken);
      if (!source || !source.record) {
        return { output: `layout not found: ${sourceToken}`, exit: false, status: "error" };
      }
      const layout = cloneWorkbenchLayoutRecord(source.record, name);
      layout.name = name;
      layout.savedAt = timestamp();
      session.savedWorkbenchLayouts[name] = layout;
      session.activeWorkbenchLayoutName = name;
      ...
      session.workbenchFocusedPanelId = optionalText(layout.focusedPanelId) ?? null;
      return {
        output: `duplicated layout ${sourceToken} as ${name}.`,
        exit: false,
        workbenchMutation: {
          kind: "save-layout",
          name,
    ...
    const layoutRenameMatch = command.match(/^layout\s+rename\s+([A-Za-z][A-Za-z0-9_-]*|current)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (layoutRenameMatch) {
      const sourceToken = layoutRenameMatch[1];
      const targetName = layoutRenameMatch[2];
      const source = resolveLayoutCatalogRecord(sourceToken);
      if (!source || !source.record || !source.name) {
        return {
          output: sourceToken === "current"
            ? "current layout has no authored name to rename."
            : `layout not found: ${sourceToken}`,
          exit: false,
          status: "error"
        };
      }
      if (source.name === targetName) {
        return { output: `layout already named ${targetName}.`, exit: false, status: "error" };
      }
      if (session.savedWorkbenchLayouts?.[targetName]) {
        return { output: `layout already exists: ${targetName}`, exit: false, status: "error" };
      }
      const layout = cloneWorkbenchLayoutRecord(source.current ? activeWorkbenchLayout() : source.record, targetName);
      layout.name = targetName;
      layout.savedAt = timestamp();
      delete session.savedWorkbenchLayouts[source.name];
      session.savedWorkbenchLayouts[targetName] = layout;
      session.activeWorkbenchLayoutName = targetName;
      clearWorkbenchLayoutOverlay({ preserveActiveName: true });
      session.workbenchFocusedPanelId = optionalText(layout.focusedPanelId) ?? null;
      return {
        output: `renamed layout ${source.name} to ${targetName}.`,
        exit: false,
        workbenchMutation: {
          kind: "rename-layout",
    ...
    const layoutSaveMatch = command.match(/^layout\s+save\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (layoutSaveMatch) {
      const name = layoutSaveMatch[1];
      if (session.savedWorkbenchLayouts?.[name] && session.activeWorkbenchLayoutName !== name) {
        return { output: `layout already exists: ${name}`, exit: false, status: "error" };
      }
      const layout = cloneWorkbenchLayoutRecord(activeWorkbenchLayout(), name);
      layout.name = name;
    ...
    const layoutOpenMatch = command.match(/^layout\s+open\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (layoutOpenMatch) {
      const name = layoutOpenMatch[1];
      const layout = cloneWorkbenchLayoutRecord(session.savedWorkbenchLayouts?.[name] ?? null, name);
      if (!layout) return { output: `layout not found: ${name}`, exit: false, status: "error" };
      session.activeWorkbenchLayoutName = name;
      clearWorkbenchLayoutOverlay({ preserveActiveName: true });
      session.workbenchFocusedPanelId = optionalText(layout.focusedPanelId) ?? null;
      return { output: `opened layout ${name}.`, exit: false };
    }

    const layoutDeleteMatch = command.match(/^layout\s+delete\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (layoutDeleteMatch) {
      const name = layoutDeleteMatch[1];
      if (!session.savedWorkbenchLayouts?.[name]) {
        return { output: `layout not found: ${name}`, exit: false, status: "error" };
      }
      delete session.savedWorkbenchLayouts[name];
      if (session.activeWorkbenchLayoutName === name) {
        clearWorkbenchLayoutOverlay({ preserveActiveName: false });
      }
      return {
        output: `deleted layout ${name}.`,
        exit: false,
        workbenchMutation: {
          kind: "delete-layout",
          name
    ...
    if (command === "layouts") {
      const current = activeWorkbenchLayout();
      const names = Object.keys(session.savedWorkbenchLayouts ?? {}).sort();
      const rows = [
        `* current${current.name ? ` (${current.name})` : ""} :: panels=${collectLayoutPanelIds(current.root).length}${hasWorkbenchLayoutOverlay(session) ? " | unsaved" : ""}`
      ];
      for (const name of names) {
        const record = session.savedWorkbenchLayouts[name];
        const active = session.activeWorkbenchLayoutName === name ? "*" : " ";
        rows.push(`${active} ${name} :: panels=${Object.keys(record?.panels ?? {}).length}${record?.viewportId ? ` | viewport=${record.viewportId}` : ""}`);
      }
      return {
        output: rows.join("\n"),
        exit: false
    ...
    if (command === "layout reset") {
      clearWorkbenchLayoutOverlay({ preserveActiveName: true });
      return {
        output: session.activeWorkbenchLayoutName
          ? `layout reset to ${session.activeWorkbenchLayoutName}.`
          : "layout reset to authored default.",
        exit: false
      };
    }

    const keymapNewMatch = command.match(/^keymap\s+new\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (keymapNewMatch) {
      const name = keymapNewMatch[1];
      if (session.savedWorkbenchKeymaps?.[name]) {
        return { output: `keymap already exists: ${name}`, exit: false, status: "error" };
      }
      const keymap = cloneWorkbenchKeymapRecord(activeWorkbenchKeymap(), name);
      keymap.name = name;
      keymap.savedAt = timestamp();
      session.savedWorkbenchKeymaps[name] = keymap;
      session.activeWorkbenchKeymapName = name;
      clearWorkbenchKeymapOverlay({ preserveActiveName: true });
      return {
        output: `created keymap ${name}.`,
        exit: false,
        workbenchMutation: {
          kind: "save-keymap",
          name,
    ...
    const keymapDuplicateMatch = command.match(/^keymap\s+duplicate\s+([A-Za-z][A-Za-z0-9_-]*|current)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (keymapDuplicateMatch) {
      const sourceToken = keymapDuplicateMatch[1];
      const name = keymapDuplicateMatch[2];
      if (session.savedWorkbenchKeymaps?.[name]) {
        return { output: `keymap already exists: ${name}`, exit: false, status: "error" };
      }
      const source = resolveKeymapCatalogRecord(sourceToken);
      if (!source || !source.record) {
        return { output: `keymap not found: ${sourceToken}`, exit: false, status: "error" };
      }
      const keymap = cloneWorkbenchKeymapRecord(source.record, name);
      keymap.name = name;
      keymap.savedAt = timestamp();
      session.savedWorkbenchKeymaps[name] = keymap;
      session.activeWorkbenchKeymapName = name;
      clearWorkbenchKeymapOverlay({ preserveActiveName: true });
      return {
        output: `duplicated keymap ${sourceToken} as ${name}.`,
        exit: false,
        workbenchMutation: {
          kind: "save-keymap",
          name,
    ...
    const keymapRenameMatch = command.match(/^keymap\s+rename\s+([A-Za-z][A-Za-z0-9_-]*|current)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (keymapRenameMatch) {
      const sourceToken = keymapRenameMatch[1];
      const targetName = keymapRenameMatch[2];
      const source = resolveKeymapCatalogRecord(sourceToken);
      if (!source || !source.record || !source.name) {
        return {
          output: sourceToken === "current"
            ? "current keymap has no authored name to rename."
            : `keymap not found: ${sourceToken}`,
          exit: false,
          status: "error"
        };
      }
      if (source.name === targetName) {
        return { output: `keymap already named ${targetName}.`, exit: false, status: "error" };
      }
      if (session.savedWorkbenchKeymaps?.[targetName]) {
        return { output: `keymap already exists: ${targetName}`, exit: false, status: "error" };
      }
      const keymap = cloneWorkbenchKeymapRecord(source.current ? activeWorkbenchKeymap() : source.record, targetName);
      keymap.name = targetName;
      keymap.savedAt = timestamp();
      delete session.savedWorkbenchKeymaps[source.name];
      session.savedWorkbenchKeymaps[targetName] = keymap;
      session.activeWorkbenchKeymapName = targetName;
      clearWorkbenchKeymapOverlay({ preserveActiveName: true });
      return {
        output: `renamed keymap ${source.name} to ${targetName}.`,
        exit: false,
        workbenchMutation: {
          kind: "rename-keymap",
          sourceName: source.name,
    ...
    const keymapSaveMatch = command.match(/^keymap\s+save\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (keymapSaveMatch) {
      const name = keymapSaveMatch[1];
      if (session.savedWorkbenchKeymaps?.[name] && session.activeWorkbenchKeymapName !== name) {
        return { output: `keymap already exists: ${name}`, exit: false, status: "error" };
      }
      const keymap = cloneWorkbenchKeymapRecord(activeWorkbenchKeymap(), name);
      keymap.name = name;
    ...
    const keymapOpenMatch = command.match(/^keymap\s+open\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (keymapOpenMatch) {
      const name = keymapOpenMatch[1];
      const keymap = cloneWorkbenchKeymapRecord(session.savedWorkbenchKeymaps?.[name] ?? null, name);
      if (!keymap) return { output: `keymap not found: ${name}`, exit: false, status: "error" };
      session.activeWorkbenchKeymapName = name;
      clearWorkbenchKeymapOverlay({ preserveActiveName: true });
      return { output: `opened keymap ${name}.`, exit: false };
    }

    const keymapDeleteMatch = command.match(/^keymap\s+delete\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (keymapDeleteMatch) {
      const name = keymapDeleteMatch[1];
      if (!session.savedWorkbenchKeymaps?.[name]) {
        return { output: `keymap not found: ${name}`, exit: false, status: "error" };
      }
      delete session.savedWorkbenchKeymaps[name];
      if (session.activeWorkbenchKeymapName === name) {
        clearWorkbenchKeymapOverlay({ preserveActiveName: false });
      }
      return {
        output: `deleted keymap ${name}.`,
        exit: false,
        workbenchMutation: {
          kind: "delete-keymap",
          name
    ...
    if (command === "keymaps") {
      const current = activeWorkbenchKeymap();
      const names = Object.keys(session.savedWorkbenchKeymaps ?? {}).sort();
      const rows = [
        `* current${current.name ? ` (${current.name})` : ""} :: bindings=${Object.keys(current.bindings).length}${hasWorkbenchKeymapOverlay(session) ? " | unsaved" : ""}`
      ];
      for (const name of names) {
        const record = session.savedWorkbenchKeymaps[name];
        const active = session.activeWorkbenchKeymapName === name ? "*" : " ";
        rows.push(`${active} ${name} :: bindings=${Object.keys(record?.bindings ?? {}).length}`);
      }
      return {
        output: rows.join("\n"),
        exit: false
    ...
    if (command === "keymap reset") {
      clearWorkbenchKeymapOverlay({ preserveActiveName: true });
      return {
        output: session.activeWorkbenchKeymapName
          ? `keymap reset to ${session.activeWorkbenchKeymapName}.`
          : "keymap reset to authored defaults.",
        exit: false
      };
    }

    const contentOpenMatch = command.match(/^content\s+open\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (contentOpenMatch) {
      return inspectWorkbenchRecordByName("content", contentOpenMatch[1]);
    }

    const contentDuplicateMatch = command.match(/^content\s+duplicate\s+([A-Za-z][A-Za-z0-9_-]*)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (contentDuplicateMatch) {
      const sourceName = contentDuplicateMatch[1];
      const targetName = contentDuplicateMatch[2];
      const contents = authoredContentCatalog();
      if (contents[targetName]) {
        return { output: `content already exists: ${targetName}`, exit: false, status: "error" };
      }
      const source = cloneAuthoredContentRecord(contents[sourceName] ?? null, sourceName);
      if (!source) {
        return { output: `content not found: ${sourceName}`, exit: false, status: "error" };
      }
      const nextSpecs = currentWorkbenchSpecs();
      nextSpecs.contents = [
        ...arrayWrap(nextSpecs.contents).filter(content => content?.id !== targetName),
        {
          ...source,
          id: targetName,
          title: source.title === sourceName ? targetName : source.title
      ...
      setOperatorWorkbenchAuthoringSpecs(state, nextSpecs);
      return {
        output: `duplicated content ${sourceName} as ${targetName}.`,
        exit: false,
        workbenchMutation: {
          kind: "save-content",
          name: targetName,
          record: cloneAuthoredContentRecord(nextSpecs.contents.find(content => content?.id === targetName) ?? null, targetName)
        }
      };
    }

    const contentRenameMatch = command.match(/^content\s+rename\s+([A-Za-z][A-Za-z0-9_-]*)\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (contentRenameMatch) {
      const sourceName = contentRenameMatch[1];
      const targetName = contentRenameMatch[2];
      const contents = authoredContentCatalog();
      if (!contents[sourceName]) {
        return { output: `content not found: ${sourceName}`, exit: false, status: "error" };
      }
      if (sourceName === targetName) {
        return { output: `content already named ${targetName}.`, exit: false, status: "error" };
      }
      if (contents[targetName]) {
        return { output: `content already exists: ${targetName}`, exit: false, status: "error" };
      }
      const nextSpecs = currentWorkbenchSpecs();
      nextSpecs.contents = arrayWrap(nextSpecs.contents)
        .filter(content => content?.id !== sourceName)
        .concat([{
          ...cloneAuthoredContentRecord(contents[sourceName], targetName),
          id: targetName,
          title: contents[sourceName]?.title === sourceName ? targetName : contents[sourceName]?.title
        }]);
      rewriteContentReferencesInSpecs(nextSpecs, sourceName, targetName);
      setOperatorWorkbenchAuthoringSpecs(state, nextSpecs);
      return {
        output: `renamed content ${sourceName} to ${targetName}.`,
        exit: false,
        workbenchMutation: {
          kind: "rename-content",
          sourceName,
    ...
    const contentDeleteMatch = command.match(/^content\s+delete\s+([A-Za-z][A-Za-z0-9_-]*)$/);
    if (contentDeleteMatch) {
      const name = contentDeleteMatch[1];
      const contents = authoredContentCatalog();
      if (!contents[name]) {
        return { output: `content not found: ${name}`, exit: false, status: "error" };
      }
      const refs = contentReferrersInSpecs(state.workbenchAuthoringSpecs, name);
      if (refs.length) {
        return {
          output: `content ${name} is still referenced by ${refs.join(", ")}`,
          exit: false,
          status: "error"
        };
      }
      const nextSpecs = currentWorkbenchSpecs();
      nextSpecs.contents = arrayWrap(nextSpecs.contents).filter(content => content?.id !== name);
      setOperatorWorkbenchAuthoringSpecs(state, nextSpecs);
      return {
        output: `deleted content ${name}.`,
        exit: false,
        workbenchMutation: {
          kind: "delete-content",
          name
    ...
    if (command === "contents") {
      const rows = workbenchContentRecords(state);
      return {
        output: rows.length
          ? rows.map(record => `${record.title} :: kind=${record.metadata?.kind ?? "detail"} | source=${record.metadata?.datasetId ?? record.metadata?.dataSource ?? "(none)"}`).join("\n")
          : "(no contents)",
        exit: false
      };
    }

    const bindMatch = command.match(/^bind\s+(\S+)\s+([A-Za-z_][A-Za-z0-9_.:-]*)$/);
    if (bindMatch) {
      const trigger = bindMatch[1];
      const actionId = bindMatch[2];
      if (!operatorWorkbenchActionSpec(state, actionId)) {
        return { output: `action not found: ${actionId}`, exit: false, status: "error" };
      }
      session.workbenchBindingOverrides[trigger] = actionId;
      return { output: `bound ${trigger} -> ${actionId}.`, exit: false };
    }

    const unbindMatch = command.match(/^unbind\s+(\S+)$/);
    if (unbindMatch) {
      const trigger = unbindMatch[1];
      if (!Object.prototype.hasOwnProperty.call(session.workbenchBindingOverrides, trigger)) {
        return { output: `binding not found: ${trigger}`, exit: false, status: "error" };
      }
      delete session.workbenchBindingOverrides[trigger];
      return { output: `unbound ${trigger}.`, exit: false };
    }

    if (/^layout(?:\s|$)/.test(command)) {
      return {
        output: "usage: layout new <name> | layout duplicate <source> <name> | layout rename <source> <name> | layout save <name> | layout open <name> | layout delete <name> | layouts | layout reset",
        exit: false,
        status: "error"
      };
    }

    if (/^keymap(?:\s|$)/.test(command)) {
      return {
        output: "usage: keymap new <name> | keymap duplicate <source> <name> | keymap rename <source> <name> | keymap save <name> | keymap open <name> | keymap delete <name> | keymaps | keymap reset",
        exit: false,
        status: "error"
      };
    }

    if (/^panel(?:\s|$)/.test(command)) {
      return {
        output: "usage: panel open <name> | panel duplicate <source> <name> | panel rename <source> <name> | panel delete <name> | panel assign-content <panel> <content> | panel close | panel focus next|prev|<id> | panels",
        exit: false,
        status: "error"
      };
    }

    if (/^content(?:\s|$)/.test(command)) {
      return {
        output: "usage: content open <name> | content duplicate <source> <name> | content rename <source> <name> | content delete <name> | contents",
        exit: false,
        status: "error"
      };
    }

    if (/^split(?:\s|$)/.test(command)) {
      return {
        output: "usage: split horizontal | split vertical",
        exit: false,
        status: "error"
      };
    }
...
export async function runOperatorTui({
  ...
  try {
    ...
    const authoringStore = await createOperatorWorkbenchAuthoringStore({
      appProject: runtimeContext.appProject
    }).catch(() => null);
    const authoredWorkbenchState = await authoringStore?.loadWorkbenchState?.().catch(() => null);
    if (authoredWorkbenchState?.layouts) {
      engine.session.savedWorkbenchLayouts = structuredClone(authoredWorkbenchState.layouts);
    }
    if (authoredWorkbenchState?.keymaps) {
      engine.session.savedWorkbenchKeymaps = structuredClone(authoredWorkbenchState.keymaps);
    }

    async function executeCommandWithPersistence(command, options = undefined) {
      const result = await engine.execute(command, options);
      const mutation = result?.workbenchMutation ?? null;
      if (mutation?.kind === "save-layout" && mutation.name && authoringStore?.saveLayout) {
        const saved = await authoringStore.saveLayout(mutation.name, mutation.record ?? {});
        engine.session.savedWorkbenchLayouts[mutation.name] = saved ?? mutation.record ?? {};
      } else if (mutation?.kind === "rename-layout" && mutation.name && mutation.sourceName && authoringStore?.renameLayout) {
        const saved = await authoringStore.renameLayout(mutation.sourceName, mutation.name, mutation.record ?? {});
        delete engine.session.savedWorkbenchLayouts[mutation.sourceName];
        engine.session.savedWorkbenchLayouts[mutation.name] = saved ?? mutation.record ?? {};
      } else if (mutation?.kind === "delete-layout" && mutation.name && authoringStore?.deleteLayout) {
        await authoringStore.deleteLayout(mutation.name);
      } else if (mutation?.kind === "save-keymap" && mutation.name && authoringStore?.saveKeymap) {
        const saved = await authoringStore.saveKeymap(mutation.name, mutation.record ?? {});
        engine.session.savedWorkbenchKeymaps[mutation.name] = saved ?? mutation.record ?? {};
      } else if (mutation?.kind === "rename-keymap" && mutation.name && mutation.sourceName && authoringStore?.renameKeymap) {
        const saved = await authoringStore.renameKeymap(mutation.sourceName, mutation.name, mutation.record ?? {});
        delete engine.session.savedWorkbenchKeymaps[mutation.sourceName];
        engine.session.savedWorkbenchKeymaps[mutation.name] = saved ?? mutation.record ?? {};
      } else if (mutation?.kind === "delete-keymap" && mutation.name && authoringStore?.deleteKeymap) {
        await authoringStore.deleteKeymap(mutation.name);
      } else if (mutation?.kind === "save-panel" && mutation.name && authoringStore?.savePanel) {
        await authoringStore.savePanel(mutation.name, mutation.record ?? {});
      } else if (mutation?.kind === "rename-panel" && mutation.name && mutation.sourceName && authoringStore?.renamePanel) {
        await authoringStore.renamePanel(mutation.sourceName, mutation.name, mutation.record ?? {});
      } else if (mutation?.kind === "delete-panel" && mutation.name && authoringStore?.deletePanel) {
        await authoringStore.deletePanel(mutation.name);
      } else if (mutation?.kind === "assign-panel-content"
        && mutation.name
        && mutation.contentName
        && authoringStore?.assignPanelContent) {
        await authoringStore.assignPanelContent(mutation.name, mutation.contentName);
      } else if (mutation?.kind === "save-content" && mutation.name && authoringStore?.saveContent) {
        await authoringStore.saveContent(mutation.name, mutation.record ?? {});
      } else if (mutation?.kind === "rename-content" && mutation.name && mutation.sourceName && authoringStore?.renameContent) {
        await authoringStore.renameContent(mutation.sourceName, mutation.name, mutation.record ?? {});
      } else if (mutation?.kind === "delete-content" && mutation.name && authoringStore?.deleteContent) {
        await authoringStore.deleteContent(mutation.name);
      }
      return result;
    }

    if (parsed.commands.length) {
      stdout.write(`${await executeBatchCommands({ execute: executeCommandWithPersistence }, parsed.commands)}\n`);
      return 0;
    }
    ...
    for await (const line of rl) {
      const result = await executeCommandWithPersistence(line);
