...
import { startOperatorBrowserExampleServer } from "../browser-example-server.js";
...
export async function createOperatorWorkbenchShell({
  ...
  preloadPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..", "preload.cjs")
} = {}) {
  ...
  const workspaceState = await settingsStore.loadWorkspace(workspaceKey);
  const displaySettings = workspaceState.displaySettings;
  ...
  const window = new BrowserWindow({
    ...
    minHeight: 760,
    show: false,
    title: browserExampleMode ? "Operator Example" : "Operator Workbench",
