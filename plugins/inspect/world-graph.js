...
export function worldGraphProjection(witnesses, { includeWitnesses = false, limitWitnesses = 36 } = {}) {
  ...
  const addNode = (id, kind, label = id, badges = [], context = null) => {
    ...
    if (prev) {
      ...
      nodeMap.set(id, {
        ...
        context: mergeNodeContext(prev.context, context, id, prev.kind, kind, nodeContext),
  ...
  for (const r of relations) {
    ...
    if (r.rel === "providesCapability") {
      addNode(r.from, inferKind(r.from, relations), r.from, [], nodeContext.get(r.from));
      const capabilityContext = capabilityContextFor(r.from, r.to, nodeContext, r.meta);
      addNode(r.to, "capability", r.to, [], capabilityContext);
      addEdge(r.from, r.to, "provides", r.witness, "capability", r.meta);
      continue;
    }
    ...
    if (isScalarSemanticRelation(r.rel) && !hasModuleDefinition(r.to, relations)) {
      addNode(r.from, inferKind(r.from, relations), r.from, [], nodeContext.get(r.from));
      continue;
    }
    addNode(r.from, inferKind(r.from, relations), r.from, [], nodeContext.get(r.from));
...
function nodeDetails(witnesses, relations, knownIds = new Set()) {
  ...
  for (const w of witnesses) {
    ...
    if (isSourceAnnotationWitness(w) && w.body?.target) {
      const d = ensure(w.body.target);
      d.sources = dedupeObjects([...(d.sources ?? []), {
        file: w.body.file,
        section: w.body.section,
        line: w.body.line ?? null,
        startLine: w.body.startLine ?? w.body.line ?? null,
        startColumn: w.body.startColumn ?? 1,
        endLine: w.body.endLine ?? w.body.line ?? null,
        ...
        witness: w.id
      }], x => `${x.file}\u0000${x.section}\u0000${x.startLine}\u0000${JSON.stringify(x.values)}`);
    }
  }
...
function witnessTargetsForGraph(witness, knownIds = new Set()) {
  ...
  if (isSourceAnnotationWitness(witness) && witness.body?.target && knownIds.has(witness.body.target)) {
...
function applySemanticLayoutContexts(relations, nodeContext, contexts) {
  ...
  const mcpServers = [...moduleKinds.entries()].filter(([, kind]) => kind === "mcpServer").map(([id]) => id);
  ...
  for (const id of mcpServers) nodeContext.set(id, "backend/runtime");
...
function looksLikeCapability(value) {
  return typeof value === "string" && (value.includes(".") || value.startsWith("capability:"));
}

function isScalarSemanticRelation(rel) {
  return rel === "usesStore" || rel === "identityField" || rel === "versionField";
}

function hasModuleDefinition(id, relations) {
  return relations.some(r => r.from === id && r.rel === "hasModuleKind");
}
...
function mergeNodeContext(previousContext, incomingContext, id, previousKind, incomingKind, nodeContext) {
  if (incomingContext && incomingContext.endsWith("/capabilities") && (previousKind === "capability" || incomingKind === "capability")) {
    return incomingContext;
  }
  return previousContext ?? incomingContext ?? nodeContext.get(id) ?? null;
}

function relationStyle(rel) {
  if (rel === "contextCapability" || rel === "hostCapability" || rel === "installsCapability" || rel === "dependsOnCapability" || rel === "providesCapability") return "capability";
  if (rel === "hasChildWidget" || rel === "hasChildSurface") return "composition";
...
function inferKind(id, relations) {
  ...
  if (moduleKind === "message") return "message";
  if (moduleKind === "entity") return "entity";
  if (moduleKind === "boundary") return "boundary";
  if (moduleKind === "type") return "type";
  if (moduleKind === "store") return "store";
  if (moduleKind === "projection") return "projection";
  if (moduleKind === "policy") return "policy";
...
export function astNodesProjection(witnesses) {
  ...
  for (const w of witnesses) {
    ...
    const node = {
      id: `ast:${section}:${target}:${startLine ?? line ?? "unknown"}`,
      file,
      section,
      line: line ?? null,
      startLine: startLine ?? line ?? null,
      startColumn: startColumn ?? 1,
      endLine: endLine ?? line ?? null,
      endColumn: endColumn ?? null,
      sourceLanguage: sourceLanguage ?? "wtoml",
