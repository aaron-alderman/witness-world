...
test("authoring-core package proposal targets lower contextual refs before authority checks", () => {
  const world = packageAuthoringRefWorld();
  const seenTargets = [];
...
test("authoring-core proposal targets execute context and stewardship writes through shared helpers", () => {
  const world = createWorld();
  applyWitnessToml(world, `
[[thing]]
actor = "system"
...
test("authoring-core DESIRE noun handlers create proposals instead of dead-end 403s", async () => {
  ...
  assert.equal(sent.some(entry => entry.kind === "gate"), false);
  assert.deepEqual(seenContexts, ["ctx.shared", "ctx.shared", "ctx.shared", "ctx.shared", "ctx.shared"]);
  assert.deepEqual(sent.map(entry => entry.status), [202, 202, 202, 202, 202]);
  assert.deepEqual(
    sent.map(entry => ({
      targetProcess: entry.body.proposal.targetProcess,
...
test("authoring-core proposal targets execute DESIRE noun writes through shared helpers", () => {
  const world = createWorld();
  const run = (proposal, body) => executeAuthoringCoreProposalTarget({
    world,
    actor: "aaron",
...
test("authoring-core proposal targets execute package noun writes through shared helpers", () => {
  const world = createWorld();
  applyWitnessToml(world, `
[[context]]
actor = "system"
...
test("authoring-core widget handlers create proposals instead of dead-end 403s for governed routes", async () => {
  ...
  assert.deepEqual(seenWidgetTargets, ["page_root", "shared_note"]);
...
test("authoring-core proposal targets execute route and serve writes through shared helpers", () => {
  const world = createWorld();
  applyWitnessToml(world, `
[[thing]]
actor = "system"
...
`);
  ...
  const route = executeAuthoringCoreProposalTarget({
    world,
    actor: "aaron",
    backendHost: "backendHost",
  ...
  const serve = executeAuthoringCoreProposalTarget({
    world,
    actor: "aaron",
    backendHost: "backendHost",
