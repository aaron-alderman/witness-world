...
- `node --test test/runtime-worker-control-contract.test.js`, `node --test --test-name-pattern "runtime server exposes a versioned worker-control descriptor for supervised runtimes" test/runtime-server.test.js`, `cargo test --manifest-path substrate/Cargo.toml -p witness-core`, and `node --test test/rust-owned-external-boundary-roadmap.test.js` now prove the descriptor version, explicit action URLs, config parsing, state JSON surfacing, descriptor parsing, and roadmap/config guardrails.
...
- [x] Supervised runtime plugin runtime-module loading no longer depends on direct Node filesystem reads/import of canonical plugin state.
...
- [x] Core-connected startup and plugin helper paths fail closed instead of silently falling back to local canonical disk when a source cannot be expressed as a witness-core source id.
...
- `src/runtime-verification-persistence.js` no longer owns its own `fetch`-based witness-core client; verification persistence now routes witness-core control-plane requests through [src/witness-core-bridge.js](../src/witness-core-bridge.js), which collapses the product-owned Rust control-plane HTTP client surface back to the shared bridge.
- `test/witness-core-bridge.test.js`, `plugins/platform/verification-persistence.test.js`, and `test/rust-owned-external-boundary-roadmap.test.js` now prove the shared bridge serializes verification-persistence requests correctly, the verification subsystem still survives reopen/restart through witness-core, and the direct control-plane fetch owner set has shrunk to `src/witness-core-bridge.js`.
...
- `test/runtime-plugin-utils.test.js` now proves plugin catalog discovery can list a plugin root, read `plugin.json`, and stat `runtime.js` through witness-core capabilities with local plugin-root filesystem reads blocked.
- `src/runtime-plugin-loader.js` now materializes plugin-owned runtime modules into `.witness-core/runtime-plugin-modules/...` from witness-core `listSourceDirectory(...)` and `readSource(...)` capability calls before importing them, so supervised/core-connected runtime-module loading no longer imports canonical plugin files directly even when the plugin runtime uses relative imports. Those writes are explicit non-canonical scratch outputs rather than canonical plugin-source mutation.
...
- `test/rust-owned-external-boundary-roadmap.test.js` now freezes the supervised plugin runtime-module path so `runtime-plugin-loader.js`, `src/dsl.js`, and `src/runtime-server.js` must keep using the witness-core materialization bridge instead of silently drifting back to canonical plugin-path imports.
...
- Together, those strict-mode, static-serving, and explicit-opt-in poller proofs close the remaining canonical filesystem blocker for [src/runtime-server.js](../src/runtime-server.js), [src/app-snapshot-manager.js](../src/app-snapshot-manager.js), [src/app-project.js](../src/app-project.js), [src/runtime-plugin-utils.js](../src/runtime-plugin-utils.js), and [src/runtime-plugin-loader.js](../src/runtime-plugin-loader.js); the only surviving `node:fs` runtime owners on those paths are explicit `.witness-core/...` scratch outputs plus the demoted operator/development-only no-core poller escape hatch.
...
- [x] Remove remaining direct canonical-source `node:fs` ownership from supervised runtime paths in [src/runtime-server.js](../src/runtime-server.js), [src/app-snapshot-manager.js](../src/app-snapshot-manager.js), [src/app-project.js](../src/app-project.js), [src/runtime-plugin-utils.js](../src/runtime-plugin-utils.js), and [src/runtime-plugin-loader.js](../src/runtime-plugin-loader.js) once every supervised startup, rebuild, and utility request path is Rust-authoritative and only explicit `.witness-core/...` scratch outputs remain local.
...
- [ ] Replace the Node `fetch`-based witness-core control-plane bridge in [src/witness-core-bridge.js](../src/witness-core-bridge.js) and its runtime consumers with a Rust-owned worker IPC transport so supervised Node compute reaches Rust through the same bounded worker channel rather than ad hoc HTTP requests.
