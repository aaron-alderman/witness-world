...
- [x] `plugins/starter/todo-starter-blueprint.json` is now the live starter blueprint asset; `plugins/starter/starter-blueprints.js` consumes it through a thin loader, `plugins/starter/runtime.js` contributes it through the starter provider, and the starter browser proof still passes against the live wired path
...
- [x] the starter-authored session/private-note/widget-editor chrome now also consumes shared generic `surface-*` primitives instead of leaving inspect-local CSS as the hidden owner of those authored surfaces: `plugins/starter/todo-starter-blueprint.json` now applies `surface-card`, `surface-stack`, `surface-status`, `surface-item-list`, `surface-item`, `surface-empty`, `surface-empty-state`, and `surface-mono` to the todo/world session panels, private-notes section, private-note rows, and widget-editor panel; `src/runtime-surface-form-controls.js` now owns the generic `value-editor-field` selector because `plugins/inspect/widget-page.js` emits that label class from the shared widget runtime; and `plugins/inspect/widget-page-styles.js` now keeps only the inspect-specific residuals for session/editor accent borders (`.session-panel`, `.widget-editor`) rather than the full shared panel/list/item/input chrome. Focused proof: `cmd /c node --test plugins\\starter\\starter.test.js test\\tutorials.test.js plugins\\inspect\\inspect.test.js` plus `cmd /c node --check src\\runtime-surface-form-controls.js plugins\\inspect\\widget-page-styles.js plugins\\starter\\starter.test.js plugins\\inspect\\inspect.test.js`
...
- [x] the live starter blueprint asset now lives in `plugins/starter/todo-starter-blueprint.json`, and `todoStarterBlueprint()` in `plugins/starter/starter-blueprints.js` is reduced to a thin live loader that returns a fresh clone per call
- [x] bootstrap no longer manually injects the starter blueprint into the starter seam; the starter provider in `plugins/starter/runtime.js` contributes the live blueprint into runtime contributions, `preferredBootstrapStarter(...)` selects the active bootstrap starter upstream, and `buildBootstrapStarterPlan(...)` now receives only `bootstrapModel`, `bootstrapState`, and the already-selected `blueprint`
- [x] the real starter request order now lives in authored `requestPlan` rows inside `plugins/starter/todo-starter-blueprint.json`, and `buildBootstrapStarterPlan(...)` plus `buildBootstrapAuthoredRequestPlanRequests(...)` interpret those rows instead of hard-coding the request sequence locally
- [x] starter existing-state elision now lives in authored `skipIfPresentIn` plus `matchField` request-plan rows inside `plugins/starter/todo-starter-blueprint.json`, and the starter plan/request-plan seams now interpret those rows generically instead of hard-coding `existingContext` / `existingRunner` branches
...
- [x] starter request-plan choreography now lives across `plugins/starter/todo-starter-blueprint.json`, `plugins/starter/starter-blueprints.js`, `plugins/starter/runtime.js`, `preferredBootstrapStarter(...)`, `plugins/bootstrap/bootstrap-contribution-state.js`, `plugins/bootstrap/bootstrap-starter-plan.js`, and `plugins/bootstrap/bootstrap-authored-request-plan.js`: the starter plugin contributes the blueprint, `buildBootstrapContributionState(...)` exposes the selected active blueprint to bootstrap state, `buildBootstrapStarterPlan(...)` supplies bootstrap host defaults, and `buildBootstrapAuthoredRequestPlanRequests(...)` interprets the authored `requestPlan` rows for the real request order plus `skipIfPresentIn` / `matchField` / `pickFields` rules
...
- [x] blueprint owner for starter creation is now explicit and file-backed: `plugins/starter/todo-starter-blueprint.json` defines the current starter semantics, `todoStarterBlueprint()` in `plugins/starter/starter-blueprints.js` is a thin live loader around that asset, `plugins/starter/runtime.js` contributes it through the starter provider, and bootstrap consumes the already-selected active blueprint from state so the shell no longer has to thread blueprint ownership manually
