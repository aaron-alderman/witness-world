...
- [L] `plugins/platform/platform-console-layout.js` now compiles that authored page tree into a stable read model, and `plugins/platform/platform-page.js` walks the authored `childSurfaces` when choosing `/platform` navigation, page titles/subtitles, section ordering, and whether authored operator forms need client behavior. Many rendered section shells now also take their visible titles, summaries, classes, and identity attributes from those authored surfaces, but section body HTML, pagination behavior, and most detailed detail-card/table rendering are still JS-backed by surface name, so this is still not full generic rendered-RVM replacement.
