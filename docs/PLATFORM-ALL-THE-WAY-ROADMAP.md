...
- [L] Card field selection, many fallback list/text card compositions, and the scalar-only long-tail value filtering rules across the console still remain JS-backed by surface name, so this is still not full generic rendered-RVM replacement.
