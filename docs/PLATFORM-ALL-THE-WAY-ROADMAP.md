...
- [L] Some fallback list/text card compositions for future or partially authored surfaces still remain JS-backed by surface name, and record shaping for authored cards still lives in JS, so this is still not full generic rendered-RVM replacement.
