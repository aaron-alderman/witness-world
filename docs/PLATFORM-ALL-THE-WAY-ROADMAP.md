...
- [L] Top-level fixed tables are now more RVM-owned too: map/profile/gap/coverage/branch-red-green/change-set-red-green surfaces author `rowFields`, and the renderer now consumes those props instead of hardcoded row assembly for those top-level non-paginated tables.
- [L] Those same fixed tables are now a bit less surface-name-dispatched too: `plugins/platform/platform-console.rvm` now gives map/profile/gap/coverage/branch-red-green/change-set-red-green surfaces authored `tableSource` identities, and `plugins/platform/platform-page.js` resolves them through one shared `renderAuthoredTableSection(...)` path instead of maintaining separate fixed-table renderers for each section.
