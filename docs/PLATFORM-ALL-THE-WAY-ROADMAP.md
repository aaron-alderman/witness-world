...
- [L] Some fallback list/text card compositions for future or partially authored surfaces still remain JS-backed by surface name, and several authored cards still rely on JS record-shaping for link/title helper objects, so this is still not full generic rendered-RVM replacement.
