...
- [L] Row extraction, query-driven sort/pagination semantics, knowledge/signal/model property-card schemas, and most card internals across the console remain JS-backed by surface name, so this is still not full generic rendered-RVM replacement.
