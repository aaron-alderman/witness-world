...
- [L] Some fallback list/text card compositions for future or partially authored surfaces still remain JS-backed by surface name, and a smaller set of authored cards still rely on JS helper shaping where the current renderer needs alternate labels or non-concept objects (for example revision/test-stream link objects and coverage rows with fallback target labels), so this is still not full generic rendered-RVM replacement.
