...
- [L] Workflow detail is now more RVM-owned internally as well: `PlatformWorkflowDetail` authors nested `PlatformWorkflowPrimaryPanel`, `PlatformWorkflowRelatedPanel`, `PlatformWorkflowSnapshotHistory`, and `PlatformWorkflowEditHistory` child surfaces, and the renderer uses those authored nested surfaces for the visible branch/change-set/proposal detail subregions. Verification, knowledge, signals, and model detail bodies, along with most table/card content and pagination behavior, remain JS-backed by surface name, so this is still not full generic rendered-RVM replacement.
