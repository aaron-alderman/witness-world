...
- [L] Workflow and verification detail are now more RVM-owned internally as well: `PlatformWorkflowDetail` authors nested `PlatformWorkflowPrimaryPanel`, `PlatformWorkflowRelatedPanel`, `PlatformWorkflowSnapshotHistory`, and `PlatformWorkflowEditHistory` child surfaces, while `PlatformVerificationDetail` authors nested `PlatformVerificationPrimaryPanel`, `PlatformVerificationRelatedPanel`, `PlatformVerificationRunHistory`, `PlatformVerificationBuildHistory`, and `PlatformVerificationBuildErrors` child surfaces. The renderer now uses those authored nested surfaces for the visible workflow and verification detail subregions.
- [L] Knowledge, signals, and model detail bodies, along with most table/card content and pagination behavior across the console, remain JS-backed by surface name, so this is still not full generic rendered-RVM replacement.
