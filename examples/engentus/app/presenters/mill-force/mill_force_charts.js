/**
 * mill-charts.js — D3 chart renderers for the Mill Force Analysis module.
 *
 * All functions are pure render functions — no state, no DOM querying.
 * SVG coordinate system: x_svg = cx + r·sin(θ)·scale, y_svg = cy + r·cos(θ)·scale
...
// Mill cross-section polar chart
...
export function drawCrossSection(svgEl, comparison, opts = {}) {
  ...
  // Cardinal labels — standard convention: 0°=East, 90°=top, 180°=West, 270°=bottom
  const cardinals = [[0, "270°"], [Math.PI / 2, "0°"], [Math.PI, "90°"], [3 * Math.PI / 2, "180°"]];
  ...
  // Rotation arrow (CCW) — small arc in top-right quadrant
...
/**
 * Physical display angle for a segment: θ=0 (6-o'clock) maps to x=180°,
 * so 0° is the centre of the chart. Returns value in [0, 360].
 */
...
export function drawForceAngleChart(svgEl, comparison, opts = {}) {
  ...
  // Physical display angles — same for both models (same phi and N_segments)
  ...
  // Charge zone: phiPrime → phi in display coords (both shifted by +180°)
  ...
  // Axes — tick labels in standard convention (0°=East): display offset is 270° from standard 0°
  const xAxis = d3.axisBottom(xSc)
    .tickValues([0, 45, 90, 135, 180, 225, 270, 315, 360])
    .tickFormat(d => `${((d - 270 + 360) % 360).toFixed(0)}°`);
  ...
  if (tipEl) {
    ...
    g.append('rect')
      ...
      .on('mousemove', function(event) {
        ...
        tipEl.innerHTML = `
          <div style="font-weight:600;margin-bottom:3px">Liner #${best.segment} &nbsp;θ=${stdDeg}°</div>
...
export function drawForceRose(svgEl, comparison, opts = {}) {
  ...
  // Cardinal labels — standard convention: 0°=East, 90°=top, 180°=West, 270°=bottom
  const cardDeg = [[0, "270°"], [90, "0°"], [180, "90°"], [270, "180°"]];
  ...
  if (tipEl) {
    ...
    svg.append('rect')
      ...
      .on('mousemove', function(event) {
        ...
        const mouseAngle = Math.atan2(dx, dy);  // θ=0 at bottom, SVG convention
        ...
        tipEl.innerHTML = `
          <div style="font-weight:600;margin-bottom:3px">Liner #${best.segment} &nbsp;θ=${stdDeg.toFixed(1)}°</div>
...
// MC P10/P90 overlay on cross-section
